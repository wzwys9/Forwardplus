...
var panelURLTransactionMu sync.Mutex
var panelConfigDirectorySync = syncDirectory
...
type heartbeatResp struct {
	...
	XrayTasks                   []json.RawMessage        `json:"xrayTasks,omitempty"`
	AcceptedXrayTaskResults     []string                 `json:"acceptedXrayTaskResults,omitempty"`
	...
	RequestXrayState            bool                     `json:"requestXrayState,omitempty"`
	RequestManagedServicesState bool                     `json:"requestManagedServicesState,omitempty"`
	PanelURL                    string                   `json:"panelUrl"`
...
func readLocalRuntimeStatePayload() localRuntimeStatePayload {
	...
	if err == nil {
		for _, f := range files {
			...
			udpPort := fxpUDPPortForLocalTunnel(tunnelID, portValue, activeFXPSpecs)
			if transportVersion == "" || udpPort == 0 {
				if !persistedFXPLoaded {
					persistedFXPEntries = loadPersistedFXPSpecs()
					persistedFXPLoaded = true
				}
				...
				if udpPort == 0 {
					udpPort = fxpUDPPortForLocalTunnel(tunnelID, portValue, persistedFXPEntries)
				}
			}
...
func fxpUDPPortForLocalTunnel(tunnelID int, port int, specs []fxpSpec) int {
	if tunnelID <= 0 || port <= 0 {
		return 0
	}
	udpPort := 0
	for _, raw := range specs {
		...
		for _, candidate := range candidates {
			...
			if udpPort != 0 && udpPort != candidate.UDPListenPort {
				return 0
			}
			udpPort = candidate.UDPListenPort
		}
	}
	return udpPort
}
...
type desiredState struct {
	...
	ConfigHash      string                       `json:"configHash,omitempty"`
	Actions         []action                     `json:"actions"`
	Xray            *XrayDesiredState            `json:"xray,omitempty"`
	ManagedServices *ManagedServicesDesiredState `json:"managedServices,omitempty"`
}
...
func (scheduler *desiredStatePushScheduler) schedule(cfg Config, push agentDesiredStatePush) {
	...
	if scheduler.running {
		var conflict *XrayDesiredState
		if scheduler.pending != nil {
			push, conflict = coalesceDesiredStatePush(scheduler.pending.push, push)
		}
...
func coalesceDesiredStatePush(pending, incoming agentDesiredStatePush) (agentDesiredStatePush, *XrayDesiredState) {
	if pending.DesiredState == nil {
		return incoming, nil
	}
	if incoming.DesiredState == nil {
		incoming.DesiredState = pending.DesiredState
		return incoming, nil
	}
	previousXray := pending.DesiredState.Xray
	incomingXray := incoming.DesiredState.Xray
	if previousXray == nil || (incomingXray != nil && incomingXray.Generation > previousXray.Generation) {
		return incoming, nil
	}
	var conflict *XrayDesiredState
	if incomingXray != nil && incomingXray.Generation == previousXray.Generation &&
		(incomingXray.SchemaVersion != previousXray.SchemaVersion || incomingXray.ConfigHash != previousXray.ConfigHash) {
		copy := *incomingXray
		conflict = &copy
	}
	desiredCopy := *incoming.DesiredState
	desiredCopy.Xray = previousXray
	incoming.DesiredState = &desiredCopy
...
func main() {
	if handled, exitCode := runManagedAmneziaWGHelperCommand(os.Args); handled {
		os.Exit(exitCode)
	}
	...
	// Managed Xray recovery is deliberately local and precedes every panel
	// authentication attempt. Control-plane failure must not gate the data plane.
	restoreManagedXrayBeforePanelAuth()
	restoreManagedServicesBeforePanelAuth()
	// Register and start the communication/reconciliation loops before restoring
	// the remaining persisted forwarding runtimes. That restore can involve many FXP listeners and
...
func setRuntimePanelURL(panelURL string) bool {
	...
	previous := normalizePanelURL(currentPanelURL(Config{}))
	updated := setRuntimePanelURLLocked(normalized)
	if !updated && previous != "" && previous != normalized && managedServicesPanelURLStableHook != nil {
		_ = managedServicesPanelURLStableHook(previous)
	}
	return updated
}

func setRuntimePanelURLLocked(panelURL string) bool {
	normalized := normalizePanelURL(panelURL)
	if normalized == "" {
		return false
	}
	...
	if previous != "" && managedServicesPanelURLPrepareHook != nil && !managedServicesPanelURLPrepareHook(previous, normalized) {
		return false
	}
	runtimePanelURL.Store(normalized)
	if previous != "" && managedServicesPanelURLStableHook != nil && !managedServicesPanelURLStableHook(normalized) {
		runtimePanelURL.Store(previous)
		return false
	}
	return true
}
...
func persistPanelURL(panelURL string) error {
	...
	if strings.TrimRight(fmt.Sprint(data["panelUrl"]), "/") == normalized {
		return panelConfigDirectorySync(filepath.Dir(path))
	}
...
func writePanelConfigAtomic(path string, content []byte) error {
	directoryPath := filepath.Dir(path)
	temporary, err := os.CreateTemp(directoryPath, ".forwardx-panel-*")
	if err != nil {
		return err
	}
	...
	if err == nil {
		err = panelConfigDirectorySync(directoryPath)
	}
	return err
}
...
func syncPanelURLFromResponse(panelURL string) {
	...
	if current == normalized {
		if managedServicesDurableHoldPresentHook() {
			if err := persistPanelURL(normalized); err != nil {
				logf("held panel URL recovery persist failed; managed service egress remains closed: %v", err)
				managedServicesDurableHoldHook()
				return
			}
			if managedServicesPanelURLStableHook != nil {
				_ = managedServicesPanelURLStableHook(normalized)
			}
		}
		return
	}
	if err := persistPanelURL(normalized); err != nil {
		logf("panel URL persist failed before runtime switch: %v", err)
		return
	}
	if !setRuntimePanelURLLocked(normalized) {
		if err := persistPanelURL(current); err != nil {
			logf("panel URL rollback persist failed; managed service egress held closed: %v", err)
			managedServicesDurableHoldHook()
...
func heartbeat(cfg Config, forceReconcile ...bool) (heartbeatResult, error) {
	...
	xrayReportedAt := time.Now().UTC()
	xrayReport := appendXrayHeartbeatState(payload, xrayReportedAt)
	managedServicesReport := appendManagedServicesHeartbeatState(payload, xrayReportedAt)
	submittedXrayTaskResults := appendXrayTaskResultsToHeartbeat(payload, xrayManagedRoot)
	...
	for _, task := range resp.XrayTasks {
		enqueueXrayTask(cfg, task)
	}
	...
	if resp.RequestXrayState || resp.RequestManagedServicesState {
		if resp.RequestXrayState {
			requestXrayStateUpload()
		}
		if resp.RequestManagedServicesState {
			requestManagedServicesStateUpload()
		}
		next := resp.NextInterval
		if next <= 0 || next > 2 {
...
func heartbeatKeepalive(cfg Config) (heartbeatResult, error) {
	...
	if resp.DesiredState != nil && resp.DesiredState.Xray != nil {
		_ = syncXrayDesiredState(resp.DesiredState.Xray)
	}
	if resp.DesiredState != nil && resp.DesiredState.ManagedServices != nil {
		_ = syncManagedServicesDesiredState(cfg, resp.DesiredState.ManagedServices)
	}
	if resp.RequestLocalState {
		requestLocalRuntimeStateUpload()
		requestXrayStateUpload()
