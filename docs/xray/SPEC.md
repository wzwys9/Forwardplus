...
- `XRAY-UI-006` 新安装和普通升级继续默认关闭 Xray 管理员界面；管理员显式执行 `migrate-forwardx` 时，如果旧部署没有 `FORWARDX_XRAY_ENABLED` 设置，迁移安装器必须将其启用。已有显式设置和本次进程环境覆盖优先，迁移与后续普通升级不得把显式关闭改为开启。
...
- `XRAY-MP-024` VMess/Shadowsocks 在固定 `v26.3.27` 中均能构建和运行，但核心会输出 deprecated 警告并建议迁移到 VLESS Encryption。ForwardX 只把它们作为兼容现有客户端的显式 profile；catalog 必须返回 `CORE_DEPRECATED` 告警，创建界面必须可见提示优先使用 VLESS/Trojan，不将其作为新建默认协议。
