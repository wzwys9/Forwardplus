...
- `XRAY-UI-007` Xray 管理内所有 Dialog（含创建、快速配置、详情、编辑、分享和确认及其嵌套窗口）不得因外部点击或焦点转移而关闭。主动关闭、取消、操作成功后的程序化关闭和既有 Escape 行为保持不变，沿用原有忙状态限制和分享材料清理；其他模块 Dialog 与 Select/Popover 不受影响。
...
- `XRAY-QC-039`（MAINT-029）正式向导顺序为域名、统一引擎、运营商路径、端口、默认线路、预览、执行。先按全局开关选择引擎，再按该引擎过滤实际入口/中转；未选择的主机不参与能力交集。切换引擎保留路径草稿，清除端口/预览证明，重新提示不兼容项，不静默换路径或引擎。
- `XRAY-QC-040` 同一逻辑路径入口可同时选择 IPv4/IPv6，共用完全相同的后续路径；提交时展开为既有两条 endpoint/DNS route，规则按 host 去重，无数据库迁移。编辑旧配置仅合并同 host、同有序后续路径的两族入口。面板登记的有效公开地址代表该主机出站能力；只有 IPv4 则无 IPv6 出站，双栈视为具备两族能力，不新增跨服务器探测。每段下一跳必须具备可用的源主机出站族；IPv4-only → 双栈中转 → IPv6-only 落地允许，直接不允许。iptables/nftables 仍禁止跨族。
- `XRAY-QC-041` 入口/中转/地址候选依据当前所有运营商路径和已选引擎动态禁用，原因标明冲突运营商/路径，移动端无需 hover。重复主机、落地中转、同 host 不同下一跳、已确定的地址族不兼容均禁止；未完成路径不得误锁合法候选，尚缺双栈桥的入口可选但需补齐中转才可提交。删除、排序、复制也重算；允许用户修正已有无效草稿，不自动篡改其他路径。
- `XRAY-QC-042` 域名确认有效期不再打断向导。已签名的过期确认只作为原授权集合的证明，在每个消费点实时重验管理员、账号/凭据/目标/编辑 revision、域名归属及完整远端记录集合后才可使用；集合变化仍拒绝并要求明确确认。未确认检查 token、端口和预览期限仍严格生效。相同域名重新检查保留路径、引擎和手填端口草稿；实际输入变化作废下游证明但不清空用户路径。无持久浏览器凭据、无限轮询或自动 DNS 写。
- `XRAY-QC-043` 已创建列表刷新只读取面板摘要，不执行同步。后台刷新静默，手动刷新才显示加载，超时可重试；存在执行中配置才轮询，全部终态停止。已有列表在后台请求失败时保留，执行中状态显示在对应配置。

- `XRAY-QC-034` 正式多跳配置为每个运营商入口增加有序 0–8 个受管中转，最终落地固定；旧入口等价于零中转。四运营商各 1–32 条路径、总入口最多 64，实际转发 host 最多 64。所有地址从服务端安全目录解析。禁止重复 host、落地主机参与中转及跨路径下一跳冲突；同 host 双栈共用 listener，下一跳地址/端口必须一致。受管落地作为唯一直达入口保留本机直达例外。
- `XRAY-QC-035` 全部转发段继续使用统一 publicPort；中间段目标为下一台所选 IP/publicPort，末段为固定落地原地址/原端口。端口和六引擎能力检查涵盖全部中转，不得静默省略中转或换引擎。DNS 只发布入口 IP；原端口默认线路直达落地，改写端口默认线路复用所选入口的完整路径。
  现有 iptables/nftables 不提供 NAT64/NAT46：任一实际转发段的入站地址族与下一跳不同，或最终落地不是明确 IP 时禁选这两项。受管落地主机使用原端口的 DIRECT 路径不产生规则，豁免该检查；改写端口后不再豁免。进程型引擎沿用既有双栈转发。UI 与服务端使用同一地址族策略，直接调用端口/preview/apply 和切换引擎也不能绕过。
- `XRAY-QC-036` 每条 DNS route 持久保存类型化 relay 地址快照；创建、编辑、同步、重试、切换引擎、详情及备份恢复共用逐段编译器，不从可能漂移的 live rule 反推期望。全部段为正式 forward_rules，归属真实端口资源，共享段去重且最后引用清理/Agent 确认后才释放端口。一致且已证明归属的 DNS 继续零写复用。
- `XRAY-QC-037` 编辑如改变活动 host/publicPort 的下一跳，不得覆盖旧 listener 或创建抢占规则，应要求新统一端口。单独切换引擎继续走既有有中断提示的 saga。全部新段 ready 后才切 DNS，失败保留旧 topology；离线、过期、并发或任一段失败都不报成功。端口和 listener ready 不等于跨服务器防火墙及端到端代理连接已验证。

- `XRAY-QC-033` 路径编排首片为独立、明确标注的交互预览，不替代当前可执行快速配置。管理员选择既有落地后，可为电信/联通/移动/教育网各设计多条 `入口 → 有序受管中转 → 固定落地` 路径，地址选择精确到主机及 IPv4/IPv6；支持复制其他线路路径、增删中转与按钮排序。DNS 示意只指向入口。草稿只在 Dialog 内存中保存，不提交端口探测、规则、DNS 或生产 preview/apply，也不显示“已生效”。同一主机当前共用监听，全部路径中该主机的下一跳必须一致（包括双地址族）；重复主机/环路、缺失或不可用 endpoint 均明确提示。每运营商最多 32 条路径、每路径最多 8 个中转。320px 手机单列、触控排序、长 IPv6 换行、内容滚动和固定底部操作必须可用，关闭草稿须确认；该独立预览的无写约束保留，正式多跳由后续已批准的 QC034–037 接通。

  受管落地主机作为唯一入口时沿用本机直达语义：界面合并为“本机直达落地”，不显示自我转发段、不计为转发服务器，且须先改选入口才能添加中转。落地主机出现在含中转的路径中仍明确拒绝；不能用地址族变化绕过重复主机检查。

- `XRAY-QC-038` DNS 管理二级视图提供“删除全部解析”：只针对当前精确子域名的全部 A/AAAA/CNAME，不受当前搜索与显示分页限制，保留 TXT/MX/NS 等其他类型和其他名称。实时只读预览展示删除/保留数量，输入完整域名确认后进入编辑并标记待删除，统一保存才生效；已有编辑须先保存或取消。在用名称、无可删除记录或读取失败时不可确认。沿用逐条 revision/在用检查，首错停止，不重放已成功删除。删空后显示正常空态；provider 返回明确零数量的空列表（含 null 容器）可规范化为空，数量/分页暂时不一致时丢弃本次结果并最多重读两次，仍不一致则报错，禁止将错误或不完整列表视作空列表。

  MAINT-030 维护：二级详情只查询当前子域名的完整记录集合，避免读取无关名称导致详情失败；既有特殊名称仍可只读查看，写入名称约束不变。校验失败保留稳定错误，服务端仅记录不含用户/provider 原文的阶段及范围，重新加载成功后正常显示空态。

- `XRAY-QC-032` DNS 管理二级视图默认查看，管理员显式进入编辑后可在内存草稿中暂存多条 A/AAAA/CNAME 新增、修改和删除；统一保存只提交差异，取消不写入。分页不得丢失待保存项目，取消含草稿的编辑须确认；编辑中禁用本卡片的返回与域名切换。复用既有逐条管理员接口和 record revision/在用名称检查；保存不承诺跨 DNSPod 原子事务，首个错误立即停止，展示成功与剩余项目、保留草稿并禁止盲目重试，核对远端后重新编辑。NS/TXT 等其他类型及系统在用名称仍只读。
...
- `XRAY-QC-027` 系统设置的 DNS 服务商页在账号卡片下增加管理员专用的“DNS 管理”。域名下拉来自当前已验证 DNSPod 账号的 zone 目录，记录列表实时从 DNSPod 读取，不在面板建立第二份通用 DNS 记录副本。第一版只允许创建、修改和删除 `A | AAAA | CNAME`，线路必须从当前 zone 的动态目录选择。列表按完整子域名聚合并在聚合后分页，进入“管理解析”二级视图查看记录。写保护限定到被未删除快速配置、有效域名 claim、未清理托管记录或活动编排引用的完整子域名；同 zone 下其他名称可写。服务端每次写前重验，改名同时检查原名与目标名，不能仅依赖界面禁用；账号删除和换绑保护不变。
...
- `XRAY-QC-029` 同步查询本地已严格校验为正整数的 DNSPod provider recordId 时，`DescribeRecord` 的精确 `InvalidParameter.RecordIdInvalid` 表示该记录在远端已不存在，必须规范化为 `DNS_PROVIDER_RECORD_NOT_FOUND` 并继续执行列表对账与补建。不得将其他 `InvalidParameter.*` 错误扩大映射为缺失，也不得因此放宽同名重复、跨配置归属或第三方记录的 fail-closed 门禁。
- `XRAY-QC-030` 编辑第一步默认使用原域名，也可显式更换。使用原域名须实时检查，只有全部远端地址记录与本配置托管 recordId/tuple/hash 吻合时可合并确认；额外或漂移记录仍要求显式确认。更换域名须确认旧域名全部托管线路 A/AAAA 将被清理，并在预览展示原/新名称和旧解析清单；新规则和新解析验证成功后才清理旧托管记录，第三方记录保留。模式和输入改变使旧 token/探测失效，保留可复用的入口、引擎与端口草稿。
...
- `XRAY-AC-019` 多 zone 账号可下拉切换；同一子域名的多个 IP/运营商解析聚合为一行，二级页面完成 A/AAAA/CNAME 增改删。系统使用的完整子域名只读，同 zone 下未占用名称可写；直接 API 写与改名到受管名称都被拒绝。远端记录全被删除时，仍在系统占用的子域名显示为零条记录且只读。
...
- `XRAY-AC-028` DNSPod 后台删除任一托管记录后，再次同步不会因旧 recordId 失效而返回 `DNS_PROVIDER_REQUEST_REJECTED`；系统会在不接管已有第三方同值记录的前提下补建精确 tuple，保存新 recordId 并完成验证。
