...
- 普通规则的多入口检查只允许浏览器提交已授权的 host/转发组、端口和协议；入口 host、网络、operation 与 reservation 均由服务端派生。聚合 check id 使用独立 HMAC context，绑定管理员、全部 `host + network + operationId`、端口、协议和短期限；解析前限制长度并拒绝非规范 base64url、未知字段、删减、重排篡改和跨管理员复用。接口只返回稳定状态与脱敏原因，不回传 Agent 原始错误或占用进程信息。
...
- **通用 DNS 管理边界：** 通用记录管理只处理当前 global binding 下的 `A/AAAA/CNAME`，不接受任意 provider action/JSON，不把远端记录落库为第二真相。写保护按账号、zone 和规范完整名称计算，涵盖非 REMOVED 配置、有效 claim、未清理托管记录及活动 operation 的新旧名称；旧解析未清理或编排未结束不得提前解锁，同 zone 未占用名称仍可写。update/delete 先回读精确 recordId 并比对 revision，写前再检查回读原名；update 还需检查目标名称，不能通过改名绕过锁。聚合分页不得使保护依赖当前显示页。非幂等 provider 写不自动重试；结果不明时只返回稳定错误并要求重读。原域名一键确认只用于远端 tuple 与本配置持有的 recordId/hash 全部吻合的记录；新增第三方记录必须显式确认。
- **通用批量删除预览：** deletionPreview 只有管理员只读权限，不授予额外写能力；只返回当前名称 A/AAAA/CNAME 的有界快照，其他类型只给保留数量。完整域名确认仅在内存形成草稿，最终逐个 remove 仍检查 revision、类型及最新占用。空列表必须来自可验证的完整 provider 响应；分页不一致的有限重读不能跨尝试拼接记录、无限翻页或重试 provider 写。
- **DNS 列表失败诊断：** `DescribeRecordList` 校验失败日志只允许固定枚举的阶段及全域/子域名范围；不输出输入名称、记录值、账号凭据、签名、原始响应、provider 原文或异常堆栈。诊断不得改变错误映射、分页预算或将无法验证的响应解释为空。只读名称过滤放宽不影响 create/update 的名称约束及任何写保护。
...
- **显式同步：** 只有管理员主动提交且 quick-config revision/current-operation CAS 通过时，维护 operation 才可重新断言 active topology。规则只修改 `xrayQuickConfigId` 明确归属本配置的行；DNS 只创建缺失托管记录，或把仍位于同一相对名称的本地 provider recordId 恢复为已持久化 tuple。本地 recordId 必须先通过正整数校验；只有 DNSPod `DescribeRecord` 的精确 `InvalidParameter.RecordIdInvalid` 可规范化为远端缺失，不得将其他参数错误当作可补建信号。创建前必须持久化已存在的同值 recordId 集合；重启接管只能认领集合外的唯一结果。移动到其他名称、未归属的同值记录、额外第三方记录和跨配置 recordId 一律不接管、不覆盖、不删除。同步失败不得以补偿名义删除原有可用数据面。
