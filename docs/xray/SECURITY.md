...
- **通用 DNS 管理边界：** 通用记录管理只处理当前 global binding 下的 `A/AAAA/CNAME`，不接受任意 provider action/JSON，不把远端记录落库为第二真相。读取时可显示在用 zone，写入时必须以服务端实时占用计数 fail closed；update/delete 还必须先回读精确 recordId 并比对客户端上次观察的规范 revision，防止跨 zone 操作和覆盖第三方变更。非幂等 provider 写不自动重试；结果不明时只返回稳定错误并要求重读。
