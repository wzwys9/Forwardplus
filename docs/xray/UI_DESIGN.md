...
状态：第一版界面已实施；多协议创建流程已批准并按 profile 垂直切片增量开放；DNSPod 快速配置、多跳路径、同步与记录管理界面已批准。与 `SPEC.md` 0.27 配套。
...
Xray 管理内的创建、详情、编辑、快速配置、账户、分享、证书、出口节点、独立服务、运行环境和错误/删除确认 Dialog 统一阻止外部点击或焦点转移导致的关闭。嵌套弹窗同样适用，点击子弹窗外区域不得关闭子弹窗或父弹窗；右上角关闭、取消、成功完成后的程序化关闭和现有 Escape 行为保持不变，继续遵循请求进行中限制与分享材料清理。该策略只用于 Xray Dialog，不改变其他模块弹窗或 Select/Popover 的正常收起行为。
...
- 已配置 DNSPod 时，在账号与目录卡片下显示独立“DNS 管理”卡片；未配置账号时不显示虚假可操作表格。
- 多个 zone 使用有标签的下拉选择，单 zone 仍显示当前名称。切换后重置页码、搜索和编辑态，不保留上一域名的 recordId。
- 一级列表按完整子域名聚合，显示名称、记录数量、类型和使用状态；搜索匹配任一解析时显示整个组的数量，聚合后分页。点击“管理解析”进入二级视图，展示该名称全部记录的类型、线路、值、TTL 和状态，可返回一级列表；两级均覆盖加载、空、错误与分页，并提供手动刷新。
- 一级保留“添加子域名”。二级默认查看，提供“编辑”入口；进入后显示“添加解析 / 保存 / 取消编辑”，单条编辑弹窗只“暂存修改”，删除弹窗只“标记删除”，并明确尚未写入 DNSPod。待保存清单列出类型、名称、线路、值、TTL、动作并支持继续编辑/撤销；跨页保存全部草稿而不是只保存当前页。相同记录不产生写入，取消编辑须确认丢弃草稿；编辑期间返回/切换域名禁用，浏览器离开提醒。保存中禁用重复提交和退出，首个错误停止且显示成功数量及剩余清单，禁止重复保存；用户确认结束并刷新后再核对远端、重新编辑，已成功项不回滚。
- 只有系统占用的完整子域名显示“系统使用中 · 只读”，该名称下禁用新增、编辑和删除；同 zone 其他未占用名称保持可写。零远端记录的受管名称仍显示并提示通过快速配置同步补齐。远端变更冲突或写结果不明时不做自动重试，提示用户刷新。
...
目标卡片保留原“配置”，并提供“路径设计（预览）”。该入口仍在管理员 Xray 与 DNS 账号门禁内；只读取既有安全主机目录，不改变正式向导或已创建配置。标题与底栏始终说明“仅交互预览，不下发”。

- 四个运营商用可换行的 2×2 手机选择器/桌面一行按钮切换，每项显示路径数。每类允许多个入口路径；从其他运营商复制路径是独立草稿，修改一份不改变另一份，下一跳冲突实时提示。
- 桌面左侧路径列表、右侧逐跳编辑；手机先显示路径列表，进入单条编辑，提供显式“返回路径列表”。逐跳卡片纵向串联，入口/中转分别选择主机及 IPv4/IPv6；入口只能改选，落地锁定，中转可增加、上移、下移、删除，不依赖拖拽或 hover。
- 底栏提供“关闭预览 / 查看路径汇总”，汇总展示四类路径和入口 DNS 示意；不得伪装为已验证端口、可执行规则或正式 DNS 差异。汇总可返回继续编辑，缺失与冲突集中展示，不显示可执行提交按钮。
- 外部点击不关闭，显式关闭/Escape 在存在草稿时确认丢弃；标题、IPv6、域名可换行，控制按钮触控高度至少 44px，选择弹层不能超过视口。弹窗内容独立滚动，底部区域考虑安全区，320×500、768、1024、1440 宽度验证。
- 本片不引入新的 API、数据库字段、依赖、Agent payload 或本地持久草稿。后续接通真实多跳时，必须先固化规则身份、逐跳端口、复用与清理契约，不能直接把草稿塞入旧的 carrier endpoints。

#### 当前正式向导

MAINT-027 接入后第二步升级为“运营商路径”，沿用已认可的手机单条编辑/桌面双栏设计，支持入口、逐跳中转及固定落地；把草稿提交回正式向导才进入引擎和端口检测。返回修改路径清除所有下游结果但保留旧探测替换凭证。引擎交集包括中转；预览逐段显示真实下一跳，详情展示完整路径和线路归属。旧配置编辑自动填为零中转。改变活动 host/port 下一跳时提示换统一端口以保护现有链路。现有仅预览入口仍明确无下发；不得把它的结构检查显示为网络验证通过。
...
编辑复用六步向导并预填 active topology；用户可以返回任一步，目标落地不可更换。域名页默认“使用原域名”，展示原 FQDN；“使用原域名并继续”执行实时检查，仅当所有地址冲突均可证明为本配置当前托管记录时合并确认并前进，否则展示额外记录要求显式确认。“更换域名”展开 zone/前缀输入，并提示最终提交后清理原域名下本配置生成的全部线路 A/AAAA，第三方记录保留，客户端需换用新地址；明确确认该提示后才允许确认新域名。模式切换清除检查/确认和下游探测结果，保留可复用的入口、引擎和端口草稿；跨 zone 的线路须重新从目录解析。检查中的旧响应不得覆盖新输入。最终预览再次展示原 FQDN、新 FQDN 和旧托管解析清理范围，先新规则 ready、再新 DNS 验证、最后清旧解析与旧规则；失败按既有编排展示部分成功。删除先展示将删除的当前托管 A/AAAA、规则和端口，明确不会恢复创建前或历史 edit 的第三方记录，并要求输入完整 FQDN；Agent 离线或 DNS 漂移时禁用提交并提供重试/处理说明，不提供“只删数据库”的绕过按钮。

`PARTIAL_FAILURE` 保留“重试当前操作”入口和逐步结果；不能把“清理干净”作为会覆盖第三方 DNS 或释放未确认端口的动作。operation 进行中阻止第二次编辑/删除。

### 16.5 普通规则与分享联动

编辑预览的 DNS 逐项显示“复用（不改写）/新建/修改”，并显示数量摘要和 TTL。一致且已证明归属本配置的记录不再进入“将修改或删除的同名记录”；该清单逐项标识实际修改或删除。全复用时说明仍会实时验证，但不会重复写入 DNSPod。此变更不增加用户步骤，最终提交仍执行服务端重验。
...
快速配置详情在“删除快速配置”前显示“同步配置”。仅 `ACTIVE` 且无 current operation 时可点击；提交后沿用 operation 轮询，按钮显示提交中，详情继续展示逐规则与逐 DNS 步骤。同步完成后自动刷新详情和列表；部分失败显示稳定错误码，但不把现有 active topology 隐藏或误报为已删除。该动作不弹出可编辑 payload，也不要求管理员重新填写向导。
