...
- 二级查看状态增加“删除全部解析”按钮；编辑中须先保存或取消才能使用。打开只读预览，明确“全部分页、全部线路的 A/AAAA/CNAME”，显示将删/保留数量并输入完整域名确认；确认仅标记删除并进入统一编辑保存，取消不写。TXT/MX/NS 等永远保留，无可删除记录和在用名称不可提交。弹窗外点击不关闭，手机内容可滚动、底栏与 44px 按钮可达；读取失败可只读重试。
...
MAINT-029 / QC039–043 优先于以下历史六步描述：域名 → 引擎 → 运营商路径 → 端口 → 默认线路 → 预览 → 执行。入口两族可多选但同路径共用后续；每个禁用候选以可换行文字说明原因。引擎更改保留草稿并重算兼容性，域名过期不弹回第一步，实时复核失败在原位置保留草稿。已创建列表只有手动刷新转圈，后台错误保留缓存列表并允许重试，全部终态不轮询。手机沿用 44px 控件、单列编辑及可滚动底栏。
