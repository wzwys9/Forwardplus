...
- 列表显示主机记录、类型、线路、值、TTL 和状态，覆盖加载、空、错误与分页状态；提供手动刷新。
- “添加记录”和“编辑记录”使用可滚动 Dialog，只提交 A/AAAA/CNAME、动态线路、主机记录、值和 TTL。删除弹窗必须明确展示完整记录。
- zone 在用时显示“已被快速配置占用，当前只读”及引用数，禁用新增、编辑、删除，但保留列表与刷新。远端变更冲突或写结果不明时不做自动重试，提示用户刷新。
