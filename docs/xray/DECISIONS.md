...
## MAINT-030：DNS 子域名读取隔离与失败诊断（2026-09-06，维护方案）

用户要求先分析线上删空后加载失败，再制定方案修改。访问日志确认多次删除成功后详情读取返回 502；现场只读查询时全域与目标空名称均正常，线上已包含 MAINT-028，未取得故障时原响应，不能将索引延迟或具体字段异常认定为已证实根因。

修正可确认的查询耦合：详情 `list` 提供 `subdomain` 时传给 DNSPod `SubDomain`，仍在本地精确过滤并校验该查询的完整分页。全域聚合与删除预览保持现有路径。只读名称接受已有的 `@`、通配符和下划线名称，限制长度和控制字符；不放宽写入名称规则。DNSPod 官方 [DescribeRecordList](https://cloud.tencent.com/document/api/1427/56166) 定义 `SubDomain` 精确过滤及 `ErrorOnEmpty=no` 空态。

列表校验失败仅记录固定枚举阶段和查询范围（全域/子域名），不记录请求、原响应、域名、记录值、凭据或异常堆栈。明确零数量才显示空态；未知或不完整响应继续失败。此维护解决查询耦合和诊断缺口，不宣称已复现或排除线上所有失败原因；不进行真实 DNS 删除重演，不部署。

## MAINT-029：引擎前置、双栈路径与无损复核（2026-09-05，已批准）

用户一次确认 QC039–043。以现有 route 展开表达双栈，不改 Agent/schema；登记地址作为出站能力，不做额外可达性探测。候选结构校验与提交校验同时覆盖全部路径，未选 host 不阻塞引擎。允许逐步补齐双栈桥，禁止提交不完整路径。

替代旧“确认超过十分钟必须返回第一步”的交互：仅 resolveConfirmedQuickConfigDomain 可把过期的已确认签名作为实时复核依据，仍验证同一用户/全量 revision/远端集合和原 action，未确认 token 及端口/preview TTL 不放宽。保留原 token 字节以维持已有 probe hash 绑定，不在客户端自行续期或自动授权新增第三方解析。刷新改为有界手动状态，后台静默。本轮只本地提交，不发布部署。

## MAINT-028：DNS 空态与按子域名批量标记删除（2026-09-05，已批准）

用户明确限定只删 A/AAAA/CNAME，其他类型保留。新增只读 deletionPreview 一次有界读取精确子域名全部记录，避免浏览器对每个显示页重复扫描整个 zone；不增加批量写接口。输入完整名称确认后复用 MAINT-025 草稿/保存及既有精确 recordId 写保护，不能按名称无条件删除，第三方并发改写导致 revision 冲突仍停止。

线上只读检查时列表及空名称均已正常，日志没有保留当时响应，因此历史根因未确认。修复代码可确定的读取缺口：明确零数量的 null/数组空态兼容、分页暂时不一致有限重读；所有重读共用原总页数上限，不接受不完整快照。不得以删除生产记录重现故障，不发布部署。

## MAINT-027：正式多跳编排（2026-09-05，已批准）

用户认可 MAINT-026 的手机/桌面路径交互并要求实现实际功能。用 route 的可选有序 relay 快照与一个共用编译器表达期望拓扑，不复制成另一套 Agent 运行时；正式规则负责实际运行。统一端口与全局账本不变，所有中转纳入检查；同 host/port 下一跳变更要求新端口，保护旧链路。旧 route 自动按零中转读取；同步、编辑、切换、重试、删除、详情、主机引用和备份恢复一并接通。六引擎沿用现有能力，不承诺端到端连通测试。分后端规划/持久化/生命周期/正式移动界面几个增量，只运行关键定向测试；不发布部署。

核对现有 Agent 的 iptables/nftables 命令后，确认未实现 NAT64/NAT46：按每个实际段检查地址族，域名落地也不猜测地址族；只豁免原端口本机 DIRECT，改写端口后再次检查。该限制使用共享纯策略和稳定错误码，不增加 Agent payload 或静默更换引擎。独立审查要求补上 DIRECT 豁免、切换引擎前的 live rule 逐段 tuple 校验及备份非法路径的稳定拒绝错误，均纳入本轮实现和关键回归。
...
MAINT-025（2026-09-05，已确认）：DNS 管理增加页面级编辑会话和统一保存，单条弹窗从立即 provider 写变成内存暂存；只对差量依次调用现有接口，保留服务端 revision/占用门禁。部分失败不伪装原子回滚，不自动重放；不新增后端批量接口、持久草稿或新的 DNS 权威数据。
...
MAINT-026（2026-09-05）：按用户“先做一版看看”先交付移动端优先的独立路径设计预览。当前 planner 的规则 target 固定为落地，不能只扩展 UI 然后沿用原 apply。首片读取现有主机安全 DTO、仅保存内存草稿并明确无下发；桌面双栏与手机列表/编辑两层共用模型，不使用拖拽画布。实际多跳执行作为下一片，未因界面预览而被声明已实现。
