...
## MAINT-030 子域名读取隔离与诊断回归

- 服务集成：全域列表校验失败、目标子域名明确为空时，详情仍成功；目标查询失败仍报错，保留本地精确过滤、搜索、分页和占用状态。全域聚合与删除预览不改变范围。
- Provider：`SubDomain` 与 `ErrorOnEmpty=no`，普通名称和已有 `@`/通配符/下划线只读兼容；控制字符拒绝、写入名称规则不放宽。计数、容器、记录字段、分页不一致和页数耗尽继续拒绝；失败日志只含固定阶段与范围，注入敏感哨兵不得泄漏。
- 浏览器：删空后首次读取返回稳定错误，重新加载成功后清除错误并显示正常空态；不得重复删除。保留既有手机批量删除与在用禁用回归。
- 现场已证实删除成功后的详情 502，未取得故障原响应；只读实测全域 47 条、目标名称 0 条均正常。真实故障字段及修复后线上效果仍待后续运行证据；本任务只做本地代码和隔离测试，不部署或重演真实 DNS 写入。
- 验证结果：先失败的三个回归（范围隔离、特殊名称读取、失败诊断）修复后通过；provider/service/repository、DNS apply/reuse/domain ownership 和草稿共 42 项通过，TypeScript、生产构建和文档构建通过。隔离 Chromium 3 项通过并检查恢复空态截图，覆盖 `200 → 删除成功 → 502 → 重读 502 → 重读 200` 且仅一笔删除；自动服务启动卡住后改为手动启动同一本地 Vite 夹具。独立审查无阻断项。未跑无关 Go/全量服务器、多数据库实例、真实 DNS 写入及线上登录态端到端验证。

## MAINT-029 引擎前置、双栈路径与无损复核关键回归

- 模型/状态/候选：同路径双栈展开为旧 DTO、旧配置按同主机及相同有序后缀合并、复制独立、每运营商 32 个物理入口及全局 64 个上限；跨运营商共享 listener 的下一跳冲突动态禁选，删除后解锁，已有三方冲突仍能逐条修复。修改引擎/域名及复核保留路径与手填端口，同时使旧端口和预览证明失效。
- 地址族/服务：仅登记 IPv4 的源不能直接转发到 IPv6；双栈中转可桥接，iptables/nftables 仍拒绝跨族。空 entries 只列全局引擎、不查询所有主机能力，选完路径再校验实际主机；规划及切换在端口写入/探测前执行相同门禁。
- 域名：只对已签名确认凭据允许过期后实时复核；用户、目标/配置/账号/绑定版本、域名占用和完整远端记录集必须仍匹配。不放宽未确认检查、端口探测、预览 TTL，不把旧确认直接当作 DNS 写授权。
- 刷新：列表独立请求，15 秒上限覆盖响应体读取，取消/超时终止且可重试；只轮询非终态配置，后台不驱动手动按钮转圈，失败保留已有卡片。
- 最小 node 检查共 40 个不同用例通过：Paths 8、Flow 9、Availability 3、共享引擎策略 3、域名归属/确认 14、列表请求 2、多跳 SQLite 集成 1。按受影响文件定向执行，没有重跑全量服务器或 Go 测试。候选计算按打开的下拉框惰性执行并缓存；200 主机/400 地址候选压力检查约 561ms 首算、0.4ms 缓存命中，仅代表本机模型开销。
- 隔离 Playwright 4 条通过：正式手机路径保存；真实向导 300px/320px 的引擎前置、双栈入口、过期确认进入端口、nftables 禁用及切换/域名复核保留草稿；列表已生效不轮询、后台静默、手动刷新失败结束并保留四卡片及重试。无横向溢出，关键按钮在视口内，截图已检查；所有 API 均为本地 mock，没有真实 DNS 写入。
- TypeScript、生产和文档构建通过；独立代码审查无阻断项。剩余检查：真实 Agent/六引擎逐段连通及防火墙、DNSPod 实际复核/切换/失败补偿、MySQL/PostgreSQL 实例、完整面板 E2E、真实手机键盘与读屏。登记双栈按产品规则视为可达，本次没有新增或声称完成网络可达探测。未推送、发版或部署。

## MAINT-028 DNS 空态与批量删除关键回归

- 已验证：provider 明确零数量的数组/null 空态、缺失或非法总数拒绝、删除期间短页/总数变化有限重读、尝试共用分页预算；永久错误不当作空，写操作不重试。
- 已验证：SQLite 服务预览一次读取全列表，105 条跨页 A/AAAA/CNAME 全部包含，TXT/MX/NS 和相似前缀其他名称保留；在用名称拒绝、逐条 revision 冲突及写前占用保护仍有效。
- 已验证：草稿确认不写、保存只删选定 ID/revision、取消零写、首错停止；隔离 Playwright 两条覆盖 320px 手机、搜索隐藏分页、全名确认、外点不关、固定底栏、25 条保存后保留其他类型、零记录空态及在用禁用。
- 类型检查、生产构建与文档构建通过；浏览器使用本地 mock tRPC 和示例域名，无真实 DNSPod 写入。
- 现场限制：只读检查 testui 时两个 zone 和空子域名均正常，未取得历史错误响应，不能认定已复现当时根因；有限重读处理可检测到的分页不一致，不承诺跨页原子快照。未执行真实删除重演、多数据库实例或无关 Go/全量测试，也未部署。

## MAINT-027 正式多跳关键回归

- 编译：零中转兼容、两跳/多运营商共享段、默认入口路径复用、双栈下一跳；环路/冲突/落地误用/超限 fail closed。
- 规划/持久化：中转纳入端口及引擎检查，编辑相同 listener 改目标必须新端口，路径变化使旧签名失效；SQLite schema 幂等及三方言字段描述。
- 生命周期：同步/重试/切换保留逐段下一跳，DNS 前所有段 ready，所有正式规则可追踪；备份嵌套 host ID 映射和主机引用保护。
- 正式向导最小手机路径编辑检查、TypeScript/生产和文档构建。无真实 DNS/Agent 写入，不跑无关 Go、全量后端和数据库实例矩阵；未覆盖项记录交接。

本轮已跑编译器、路径草稿/Flow、引擎地址族、schema、既有 DNS 复用/sync、备份保护及 SQLite 多跳同步集成。集成覆盖创建缺失的全部正式段、下一跳漂移修复不重复建规则、切换前拒绝漂移、全局端口引用及无 live rule 时的中转主机删除保护。手机正式入口使用 320×640 隔离夹具检查冲突禁用、路径保存和后续向导保留中转顺序；不是连接真实服务端的端到端测试。

后置清单（按用户要求不扩大全量验证）：真实 Agent/六引擎逐跳连通及防火墙、DNSPod 实际切换和失败补偿、并发编辑/崩溃恢复、改下一跳换端口全链路、备份导入嵌套 host ID 的真实重映射、MySQL/PostgreSQL 实例迁移、完整面板 E2E/真实手机系统键盘与读屏。不能把规则 listener ready 当作这些检查已通过。
...
- 所有高风险故障测试通过。
- 功能开关默认策略和升级说明完成；聚焦目标证明新安装/未迁移普通升级默认关闭、显式 `migrate-forwardx` 缺省开启；首跳旧脚本删除显式字段后，持久迁移标记只在字段完全缺失时恢复界面，两套新版安装器会把恢复值持久化。已有、空、非法或进程环境显式关闭值始终优先，Docker Compose 将规范值传入容器。
...
6. **全局端口账本**：跨 host、TCP/UDP、并发申请、同 quick-config 多 host alias、目标 Xray 原端口 alias、低端口历史回填、tunnel/mimic/exit/hop/managed service 覆盖、`LEGACY_CONFLICT`、崩溃恢复和 12 小时 lease/cohort 保守回收；迁移后的旧普通转发/隧道监听必须在另一 host 的 Xray 手动选择、自动候选和 Agent 成功结果复核中全部被排除。
...
8. **普通规则源端口门禁**：直连与转发组同步检查覆盖跨主机非 `FREE` allocation，随机候选排除同一集合；新建表单按完整入口 host 和 `tcp|udp|both` fan-out，任一 bind 占用/离线/能力不足/超时不得可用。聚合 check id 覆盖篡改、删减、跨管理员、过期和重复输入清理；最终 create 的全局 allocation 竞态保护保持有效。编辑自身端口只允许稳定 owner，不执行自身 bind 探测。
...
## 77. MAINT-023 Xray Dialog 外部交互防误关（2026-09-05）

- 单元回归覆盖 Xray 专用内容组件阻止 outside event 的默认关闭，保留消费者回调、ref、样式、内容和 Escape 处理，继续复用基础 Dialog 的主动关闭路径。
- 接入检查覆盖全部 Xray Dialog 消费组件，禁止直接导入全站默认可外部关闭的内容组件，避免遗漏或新窗口回退。
- 聚焦运行 Dialog、节点删除确认和运行环境确认三个测试文件及 TypeScript；浏览器可用时检查嵌套弹窗外部点击、右上角关闭、取消和下拉菜单。无浏览器连接时明确记录未执行真实点击，不扩大为全量业务或线上测试。
