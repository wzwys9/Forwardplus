...
MAINT-027 已接通正式多跳快速配置：运营商路径可选择入口和有序中转，支持手机编辑、逐段规则和详情回显；旧直达配置兼容。已完成关键隔离检查，尚未部署及验证真实跨服务器链路，剩余验证见 `TEST_PLAN.md`。

MAINT-029 已完成本地实现：统一引擎前置到运营商路径之前，一条路径可同时发布 IPv4/IPv6 入口并共享后续路径；按登记地址、引擎和其他路径动态禁用冲突候选。过期的已确认域名在服务端实时复核，不重置用户草稿；列表后台静默刷新、手动刷新有界且失败保留缓存。关键移动端和服务端检查已通过，未部署，真实 DNS/Agent 链路验证仍后置。
