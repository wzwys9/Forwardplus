...
Xray 管理界面由 `FORWARDX_XRAY_ENABLED` 保护并默认开启。当前策略下，字符串 `1`、`true` 或 `on`（忽略大小写和首尾空白）表示开启，其他字符串值表示关闭；字段缺失时也默认开启。安装器同时维护内部标记 `FORWARDPLUS_XRAY_UI_POLICY_VERSION=1`。

早期从原版 ForwardX 迁移的部署可能被旧安装器写入 `FORWARDX_XRAY_ENABLED=0`，也可能没有后来才增加的迁移标记，导致 Xray 入口在反复升级后仍被隐藏。新版不再依赖原迁移标记：如果 Xray UI 策略版本缺失，面板会立即按开启显示，安装器在本次升级中把开关与策略版本一起持久化为 `1`。由于旧文件没有记录 `0` 的来源，策略升级前手工设置的 `0` 也会被开启这一次；策略版本写入后再设置为 `0`、`false` 或 `off`，后续升级会尊重并保留关闭选择。公共设置查询失败仍保持关闭。
...
升级旧面板只迁移上述界面开关策略，不会改写已有转发配置。旧 Agent 没有 Xray v1 capability 时会显示“需要升级 Agent”，不会收到 Xray 安装或配置；新 Agent 对旧面板保持向后兼容。建议先升级面板，再逐台升级 Agent，确认能力和制品状态。
