...
Xray 管理界面由 `FORWARDX_XRAY_ENABLED` 保护。新安装和未迁移环境的普通升级默认关闭；使用 `migrate-forwardx` 从原版迁移时，如果原部署没有显式设置该变量，安装器会自动写入 `1`，后续升级继续保留。若第一次跨版本升级仍由进程中已加载的旧脚本重写 `.env` 并删除该字段，面板会依据持久 `FORWARDPLUS_MIGRATE_AGENTS=true` 恢复界面，新版安装器在下次写环境时将其持久化。已有显式设置优先，不会把管理员主动关闭改为开启。只有字符串 `1`、`true` 或 `on`（忽略大小写和首尾空白）显式启用；字段存在但为空、拼写错误或为关闭值时保持关闭，公共设置读取失败也保持关闭。
