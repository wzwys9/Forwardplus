...
| `FORWARDX_XRAY_ENABLED` | 关闭 | 管理员 Xray UI 功能开关；仅 `1`、`true`、`on`（忽略大小写和空白）开启，其他值失败关闭。新安装和普通升级默认关闭；`migrate-forwardx` 在原部署没有显式设置时自动开启并由后续升级保留。详见[受管 Xray 节点](./xray.md)。 |
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `FORWARDX_IMAGE` | `ghcr.io/wzwys9/forwardplus:latest` | Docker 部署使用的镜像。 |
| `FORWARDX_IMAGE_REPO` | `ghcr.io/wzwys9/forwardplus` | Docker 一键脚本解析版本镜像时使用的镜像仓库。 |
| `FORWARDPLUS_GITHUB_TOKEN` | 空 | 可选的 GitHub 只读 Token，用于提高更新检查和 Release 下载的 API 限额；只配置在服务端，建议限定为本仓库 `Contents: read`。 |
| `FORWARDPLUS_MIGRATE_AGENTS` | `false` | `migrate-forwardx` 动作写入；开启后，面板会把来源不是 Forwardplus 的已注册 Agent 在重连时加入迁移队列。 |
...
`FORWARDX_GITHUB_ACCELERATOR_URL` 使用 `加速站地址/原始 GitHub URL` 的代理格式，供安装和升级脚本读取。面板内的更新功能需另行在「系统设置 -> 系统信息 -> GitHub 下载加速」填写地址并开启「面板更新使用加速站」；开启后，版本检查、Release 信息、安装包检测、回退和升级命令会优先使用该地址，失败时自动回退直连 GitHub。`FORWARDPLUS_GITHUB_TOKEN` 只会发送给 HTTPS 的 GitHub 官方域名，不会发送给加速站。
