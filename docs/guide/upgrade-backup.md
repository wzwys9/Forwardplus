...
升级前建议备份数据库，并查看 [GitHub Release](https://github.com/wzwys9/Forwardplus/releases) 或项目更新日志，确认是否包含面板、Agent 或 Android 客户端更新。
...
如果已经启用受管 Xray，优先在“系统设置 → 备份与恢复”导出密码加密完整备份。该格式会用 AES-256-GCM 认证加密数据库快照，并在密文载荷内包装 Xray 面板主密钥；备份正文没有明文主密钥、Reality 私钥、客户端 UUID 或分享链接。恢复时需要原备份密码。

原始数据库导出不会包含 Xray 面板主密钥。使用前面的 `cp`、`mysqldump` 或 `pg_dump` 时，还必须通过独立安全渠道备份密钥文件：

```bash
# 本地 systemd 默认路径
sudo install -m 0600 /opt/forwardx-panel/data/xray-master.key /root/forwardx-xray-master.key.bak

# Docker 默认持久卷内路径（按实际 Compose 服务名调整）
docker cp forwardx-panel:/data/xray-master.key ./forwardx-xray-master.key.bak
chmod 600 ./forwardx-xray-master.key.bak
```

恢复原始数据库时先停止面板，把匹配的密钥恢复到原路径并保持 `0600`，再启动面板。缺少或使用错误密钥时，Xray 读取/迁移会明确返回 `SENSITIVE_DATA_UNAVAILABLE`；不要删除密钥文件后重启尝试“修复”，这不能解密旧数据。

密码加密完整备份恢复到没有 Xray 密文的新面板时会安全安装包装的密钥。目标面板已经有 Xray 密文时只允许同一密钥，冲突会终止导入且不覆盖当前密钥。旧格式加密备份若含 Xray 密文但未包装密钥，必须先独立恢复正确密钥。
...
启用 Xray 的部署建议先升级面板，再逐台升级 Agent。旧 Agent 不声明 Xray capability，面板不会向其下发 Xray 任务；升级 Agent service 可能让受管 Xray 短暂中断，新 Agent 会在面板认证前从本地 last-good 恢复。升级不会覆盖系统自行安装的 Xray。
...
升级前建议查看 [GitHub Release](https://github.com/wzwys9/Forwardplus/releases) 或项目更新日志，确认是否包含面板、Agent 或 Android 客户端更新。
