...
IMAGE_REPO="${FORWARDX_IMAGE_REPO:-ghcr.io/wzwys9/forwardplus}"
...
release_tag_from_url() {
  ...
  curl -fsSL --retry 3 --connect-timeout 10 "${curl_args[@]}" "$url" \
...
load_existing_env() {
  ...
  value="$(get_env_value FORWARDPLUS_GITHUB_TOKEN || true)"
  if [ -z "$GITHUB_TOKEN" ] && [ -n "$value" ]; then GITHUB_TOKEN="$value"; fi
...
ensure_xray_master_key() {
  ...
  if ! printf '%s\n' "$generated" | docker run --rm -i -v "$(data_volume_name):/data" busybox sh -eu -c '
    key_file=/data/xray-master.key
    IFS= read -r generated
    ...
    temporary="$(mktemp /data/xray-master.key.tmp.XXXXXX)"
