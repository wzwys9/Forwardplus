...
import DnsProviderSettingsCard from "@/components/DnsProviderSettingsCard";
...
const docsBaseUrl = "https://github.com/wzwys9/Forwardplus/blob/main/docs";
...
function panelVersionCommand(command: string, targetVersion: string) {
  ...
  if (!/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(target)) return command;
  return `FORWARDX_TARGET_VERSION=${target} ${command}`;
}
...
const settingsTabs = ["system", "dns", "telegram", "email", "personalization", "backup", "logs"] as const;
type SettingsTab = typeof settingsTabs[number];
const settingsTabItems = [
  { value: "system", label: "系统配置", icon: Settings2 },
  { value: "dns", label: "DNS 服务商", icon: Cloud },
  ...
  return (
    <div className="rounded-lg border border-primary/15 bg-primary/5 p-4">
      <div className="flex items-center justify-between gap-3 text-sm">
        ...
        {progress.percent !== null && <span className="shrink-0 tabular-nums">{progress.percent}%</span>}
      </div>
      {progress.percent !== null && <Progress value={progress.percent} className="mt-3" />}
...
function BackupRestoreSection({ panelUrl }: { panelUrl: string }) {
  ...
  const handleExportBackup = () => {
    ...
    setExportProgress({
      percent: null,
      step: "正在请求服务器生成备份",
      detail: "服务器会完成数据读取、裁剪和加密；当前接口不流式返回内部阶段。",
      status: "running",
    });
  ...
  const confirmImportBackup = () => {
    setImportProgress({
      percent: null,
      step: "正在验证并恢复备份",
      detail: importFilename ? `服务器正在校验密码、数据格式和 ${importFilename} 中的密钥材料；当前接口不流式返回内部阶段。` : "服务器正在校验密码、数据格式和密钥材料；当前接口不流式返回内部阶段。",
      status: "running",
    });
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="border-border/40 bg-card/60 backdrop-blur-md">
          <CardHeader>
            ...
            <CardDescription>
              导出一份离线备份文件；数据库内容和 Xray 面板主密钥会一起由备份密码加密保护。
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Alert>
              ...
              <AlertDescription>备份文件不保存明文主密钥或明文凭据；忘记密码将无法解密恢复。</AlertDescription>
            </Alert>
      ...
      <Dialog open={showImportConfirm} onOpenChange={setShowImportConfirm}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              导入后会接管备份内已有主机。v2 备份包含 Xray 密钥时会先验证再恢复；旧 v1 备份若含 Xray 密文，则目标面板必须已有匹配密钥。
            </DialogDescription>
          </DialogHeader>
          <Alert>
            <ShieldCheck className="h-4 w-4" />
            ...
            <AlertDescription>
              {hasExistingData
                ? "当前面板已有数据会被保留，备份内数据会增量追加；如果两端已有 Xray 密文但主密钥不一致，导入会停止且不会覆盖当前密钥。"
                : "当前面板没有业务数据，导入后会保留当前管理员账户；仅当备份实际包含安全包装的 Xray 主密钥时才会恢复该密钥。"}
            </AlertDescription>
          </Alert>
          <DialogFooter>
