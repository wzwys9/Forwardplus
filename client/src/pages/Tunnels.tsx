...
function TunnelsContent() {
  ...
  const headerStat = activeSection === "chains"
    ...
    : activeSection === "ports"
      ? {
          value: `${Number(forwardGroupSummaryQuery.data?.enabledItems || 0)} / ${Number(forwardGroupSummaryQuery.data?.totalItems || 0)} 已启用`,
          loading: forwardGroupsLoading,
          cacheKey: "tunnels.header.portsActive",
          fallback: "0 / 0 已启用",
          iconClass: "text-primary",
        }
      : activeSection === "groups"
  ...
  const linkSearchStats = activeSection === "ports"
    ? {
        filtered: Number(forwardGroupSummaryQuery.data?.totalItems || 0),
        total: Number(forwardGroupSummaryQuery.data?.scopeTotalItems || 0),
        unit: "项",
      }
    : activeSection === "chains"
