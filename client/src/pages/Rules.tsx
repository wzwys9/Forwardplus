...
const quickConfigLineLabels: Record<string, string> = {
  DEFAULT: "默认",
  TELECOM: "电信",
  UNICOM: "联通",
  MOBILE: "移动",
...
function getRuleQuickConfigRef(rule: any): RuleQuickConfigRef | null {
  const ref = rule?.quickConfigRef;
  const id = Number(ref?.id || 0);
  if (!Number.isSafeInteger(id) || id <= 0) return null;
  return {
    id,
    fqdn: String(ref.fqdn || ""),
    targetName: String(ref.targetName || ""),
    lineCategories: Array.isArray(ref.lineCategories) ? ref.lineCategories.map(String) : [],
    operationState: String(ref.operationState || ""),
  };
}

function QuickConfigRuleRefView({ rule, compact = false }: { rule: any; compact?: boolean }) {
  const ref = getRuleQuickConfigRef(rule);
  if (!ref) return null;
  const lines = ref.lineCategories.map((line) => quickConfigLineLabels[line] || line).join(" / ") || "线路同步中";
  const state = quickConfigStateLabels[ref.operationState] || ref.operationState || "状态未知";
  return (
    <div
      className={cn(
        "min-w-0 rounded-md border border-sky-500/25 bg-sky-500/5 text-sky-800 dark:text-sky-200",
        compact ? "mt-1.5 px-2 py-1" : "px-2.5 py-2",
      )}
      title={`快速配置：${ref.fqdn || "域名同步中"}；目标：${ref.targetName || "目标不可用"}；线路：${lines}；状态：${state}`}
    >
      <div className="flex min-w-0 flex-wrap items-center gap-1.5">
        <Badge variant="outline" className="h-5 shrink-0 border-sky-500/35 bg-background/70 px-1.5 text-[10px] text-sky-700 dark:text-sky-300">
          <Zap className="mr-1 h-3 w-3" aria-hidden="true" />快速配置
        </Badge>
        <span className="min-w-0 truncate font-mono text-[11px]">{ref.fqdn || "域名同步中"}</span>
        <span className="shrink-0 text-[10px] text-sky-700/75 dark:text-sky-300/75">{state}</span>
      </div>
      <div className={cn("mt-1 min-w-0 text-[10px] text-muted-foreground", compact ? "truncate" : "flex flex-wrap gap-x-3 gap-y-0.5")}>
        <span className="truncate">目标：{ref.targetName || "目标不可用"}</span>
        <span className="truncate">线路：{lines}</span>
      </div>
    </div>
  );
}
...
type RuleFormData = {
  ...
  sourcePort: number;
  targetMode: "manual" | "external";
  targetExternalProxyNodeId: number | null;
  targetIp: string;
  targetPort: number;
...
function RulesContent() {
  ...
  const [managedRuleAction, setManagedRuleAction] = useState<{ rule: any; action: "edit" | "toggle" | "delete" } | null>(null);
  const [relayShareRule, setRelayShareRule] = useState<any | null>(null);
  const [relayShareUri, setRelayShareUri] = useState("");
  ...
  const externalProxyQuery = trpc.xray.externalProxyNodes.list.useQuery(
    ...
    { enabled: user?.role === "admin" && showDialog, retry: false, staleTime: 30_000, refetchOnWindowFocus: false },
  );
  const selectedExternalProxyQuery = trpc.xray.externalProxyNodes.detail.useQuery(
    { id: Number(form.targetExternalProxyNodeId || 1) },
    { enabled: user?.role === "admin" && showDialog && form.targetMode === "external" && !!form.targetExternalProxyNodeId, retry: false, staleTime: 30_000, refetchOnWindowFocus: false },
  );
  const relayShareMutation = trpc.xray.externalProxyNodes.share.useMutation();
  ...
  const toggleRuleEnabled = async (rule: any, checked: boolean) => {
    ...
    if (getRuleQuickConfigRef(rule)) {
      setManagedRuleAction({ rule, action: "toggle" });
      return;
    }
    if (
      checked &&
  ...
  const renderRuleEnabledSwitch = (rule: any) => {
    ...
    if (getRuleQuickConfigRef(rule)) {
      return renderUnsupportedHint(
        <span className="inline-flex shrink-0">
          <Switch
            checked={enabled}
            onCheckedChange={() => setManagedRuleAction({ rule, action: "toggle" })}
            className="scale-75"
            aria-label={`查看快速配置后${enabled ? "停用" : "启用"}规则 ${rule.name || ""}`}
          />
        </span>,
        "此规则由快速配置统一管理，点击查看 DNS 与转发拓扑影响",
      );
    }
  ...
  const openEdit = (rule: any) => {
    if (getRuleQuickConfigRef(rule)) {
      setManagedRuleAction({ rule, action: "edit" });
      return;
    }
    ...
    setForm({
      ...
      targetMode: rule.externalProxy ? "external" : "manual",
      targetExternalProxyNodeId: rule.externalProxy ? Number(rule.externalProxy.id) : null,
  ...
  const requestDeleteRule = (rule: any) => {
    if (getRuleQuickConfigRef(rule)) {
      setManagedRuleAction({ rule, action: "delete" });
      return;
    }
    setDeleteRule(rule);
  };
  ...
  const externalProxyNodes = useMemo(() => {
    const items = [...(externalProxyQuery.data?.items ?? [])];
    const selected = selectedExternalProxyQuery.data;
    if (selected && !items.some((item) => Number(item.id) === Number(selected.id))) items.unshift(selected);
    return items;
  }, [externalProxyQuery.data?.items, selectedExternalProxyQuery.data]);
  const selectedExternalProxy = form.targetExternalProxyNodeId
    ? externalProxyNodes.find((node) => Number(node.id) === Number(form.targetExternalProxyNodeId)) ?? null
    : null;
  const externalTargetRouteCompatible = user?.role === "admin" && (
    !isForwardGroupRouteMode || isExternalProxyForwardType(effectiveRouteForwardType)
  );
  const setTargetMode = (mode: "manual" | "external") => {
    if (mode === form.targetMode) return;
    if (mode === "manual") {
      setExternalProxySearch("");
      setForm((prev) => ({ ...prev, targetMode: "manual", targetExternalProxyNodeId: null, targetIp: "", targetPort: 0 }));
      return;
    }
    if (user?.role !== "admin") return;
    const directForwardType = isExternalProxyForwardType(effectiveRouteForwardType)
      ? effectiveRouteForwardType
      : usableForwardTypes.find(isExternalProxyForwardType);
    if (!externalTargetRouteCompatible || !directForwardType) {
      toast.error("出口节点目标只支持六种本地工具的 TCP 转发；请先选择兼容的端口转发资源");
      return;
    }
    const node = selectedExternalProxy ?? externalProxyNodes[0];
    if (!node) {
      toast.error(externalProxyQuery.isLoading ? "出口节点正在加载，请稍后重试" : "请先在 Xray 管理中导入出口节点");
      return;
    }
    setForm((prev) => ({
      ...prev,
      ...
      targetIp: node.address,
      targetPort: node.port,
      forwardType: isForwardGroupRouteMode ? prev.forwardType : directForwardType,
      protocol: "tcp",
      proxyProtocolSend: false,
      proxyProtocolExitSend: false,
      failoverEnabled: false,
  ...
  useEffect(() => {
    if (form.targetMode !== "external" || externalProxyQuery.isLoading || selectedExternalProxyQuery.isLoading) return;
    const node = externalProxyNodes.find((item) => Number(item.id) === Number(form.targetExternalProxyNodeId));
    if (!node || (isForwardGroupRouteMode && !isExternalProxyForwardType(effectiveRouteForwardType))) {
      setForm((prev) => ({ ...prev, targetMode: "manual", targetExternalProxyNodeId: null, targetIp: "", targetPort: 0 }));
      if (!node) toast.error("原出口节点已不可用，请重新选择目标");
      return;
    }
    if (form.targetIp !== node.address || form.targetPort !== node.port || form.protocol !== "tcp" || form.proxyProtocolSend || form.proxyProtocolExitSend) {
      setForm((prev) => ({
        ...prev,
        targetIp: node.address,
        targetPort: node.port,
  ...
  }, [effectiveRouteForwardType, externalProxyNodes, externalProxyQuery.isLoading, form.protocol, form.proxyProtocolExitSend, form.proxyProtocolSend, form.targetExternalProxyNodeId, form.targetIp, form.targetMode, form.targetPort, isForwardGroupRouteMode, selectedExternalProxyQuery.isLoading]);
  ...
  const handleBatchEditRules = async () => {
    ...
    const managedRule = copySelectedRules.find((rule: any) => getRuleQuickConfigRef(rule));
    if (managedRule) {
      setShowCopyDialog(false);
      setManagedRuleAction({ rule: managedRule, action: "edit" });
      return;
    }
  ...
  const handleBatchDeleteRules = async () => {
    ...
    if (managedRule) {
      setShowCopyDialog(false);
      setManagedRuleAction({ rule: managedRule, action: "delete" });
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (form.targetMode === "external" && (!form.targetExternalProxyNodeId || !selectedExternalProxy)) {
      toast.error("请选择有效的出口节点");
      return;
    }
    if (form.targetMode === "external" && (!isExternalProxyForwardType(submitForwardType) || form.protocol !== "tcp")) {
      toast.error("出口节点目标只支持六种本地工具的 TCP 转发");
      return;
    }
    ...
    if (editingId) {
      updateMutation.mutate({
        ...
        ...(user?.role === "admin" ? { targetExternalProxyNodeId: form.targetMode === "external" ? form.targetExternalProxyNodeId : null } : {}),
        ...(form.targetMode === "external" ? { proxyProtocolSend: false, proxyProtocolExitSend: false } : {}),
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <QuickConfigRuleRefView rule={rule} compact />
      ...
      <div className="min-w-0 space-y-1.5 font-mono text-xs">
        ...
        <div className={panelClass}>
          ...
          <div className="space-y-1">
            ...
            {rule.externalProxy && <Badge variant="outline" className="h-5 max-w-full px-1.5 text-[10px]" title={`出口节点：${rule.externalProxy.name}（${rule.externalProxy.protocol}）`}><span className="truncate">出口节点：{rule.externalProxy.name}（{rule.externalProxy.protocol}）</span></Badge>}
          </div>
        </div>
      </div>
    );
  ...
  const renderTableTransferExit = (rule: any) => {
    ...
    return (
      <div className="flex min-w-0 flex-col items-start gap-1 font-mono text-[11px] leading-5">
        ...
        {rule.externalProxy && <span className="max-w-full truncate text-[10px] text-muted-foreground" title={`${rule.externalProxy.name}（${rule.externalProxy.protocol}）`}>出口节点：{rule.externalProxy.name}（{rule.externalProxy.protocol}）</span>}
      </div>
    );
  };
  ...
  const generateRelayShare = async () => {
    if (!relayShareRule?.externalProxy) return;
    setRelayShareUri("");
    try {
      const result = await relayShareMutation.mutateAsync({ id: Number(relayShareRule.externalProxy.id), relayRuleId: Number(relayShareRule.id) });
      setRelayShareUri(result.uri);
    } catch (error) {
      setRelayShareUri("");
      toast.error(error instanceof Error && error.message === "EXTERNAL_PROXY_REFERENCE_INVALID" ? "该规则当前不能生成中转链接，请确认规则已启用且入口主机有公网地址" : "中转链接生成失败");
    } finally {
      relayShareMutation.reset();
    }
  };

  const closeRelayShare = () => {
    setRelayShareUri("");
    relayShareMutation.reset();
    setRelayShareRule(null);
  };

  const copyRelayShare = async () => {
    if (!relayShareUri) return;
    try {
      await navigator.clipboard.writeText(relayShareUri);
      toast.success("中转链接已复制，并已从页面内存清除");
    } catch {
      toast.error("复制失败，请手动复制后关闭窗口");
      return;
    }
    setRelayShareUri("");
  };
  ...
  const renderRuleActions = (rule: any) => {
    ...
    return (
      <div className="flex items-center justify-end gap-1 whitespace-nowrap">
        {user?.role === "admin" && rule.externalProxy && !rule.forwardGroupId && (
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setRelayShareUri(""); setRelayShareRule(rule); }} title="生成中转链接"><ClipboardCopy className="h-3.5 w-3.5" /></Button>
        )}
        <Button
          variant="ghost"
          size="icon"
    ...
    return (
      ...
      >
        <CardContent className="action-card-content space-y-3 p-4">
          ...
          <QuickConfigRuleRefView rule={rule} />
  ...
  return (
    <div className="space-y-6">
      ...
      {relayShareRule?.externalProxy && (
        <Dialog open onOpenChange={(open) => { if (!open) closeRelayShare(); }}>
          <DialogContent className="max-h-[90svh] max-w-2xl overflow-y-auto">
            <DialogHeader>
              <DialogTitle>生成中转链接</DialogTitle>
              <DialogDescription>链接只把原出口节点的连接地址替换为当前规则入口，VLESS/SS/SOCKS5 凭据和安全参数保持不变。</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <dl className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-lg border p-3"><dt className="text-xs text-muted-foreground">出口节点</dt><dd className="mt-1 text-sm">{relayShareRule.externalProxy.name}（{relayShareRule.externalProxy.protocol}）</dd></div>
                <div className="rounded-lg border p-3"><dt className="text-xs text-muted-foreground">入口 / 监听端口</dt><dd className="mt-1 break-all font-mono text-sm">{getRuleTransferDisplay(relayShareRule).entryAddress || "入口公网地址待服务端确认"}</dd></div>
              </dl>
              {!relayShareUri ? <p className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm">只有已启用的六种本地工具 TCP 直连规则且入口主机具备公网地址时才能生成。链接不会保存到数据库或浏览器缓存。</p> : <div className="space-y-1.5"><Label htmlFor="rule-relay-share-uri">中转链接</Label><Textarea id="rule-relay-share-uri" readOnly value={relayShareUri} rows={5} spellCheck={false} className="font-mono text-xs" /></div>}
              <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={closeRelayShare}>关闭</Button>{relayShareUri ? <Button type="button" onClick={() => { void copyRelayShare(); }}><Copy className="mr-1.5 h-4 w-4" />复制并清除</Button> : <Button type="button" disabled={relayShareMutation.isPending} onClick={() => { void generateRelayShare(); }}>{relayShareMutation.isPending ? "正在生成…" : "确认生成"}</Button>}</div>
            </div>
          </DialogContent>
        </Dialog>
      )}
          ...
          <div className="min-h-0 flex-1 scroll-pb-28 space-y-3 overflow-y-auto pb-5 pr-1">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={form.protocol}
                  disabled={form.targetMode === "external"}
                ...
                <div className="space-y-2">
                  ...
                  {!routeModeLocked && form.routeMode === "local" ? (
                    ...
                    >
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {usableForwardTypes.filter((t) => form.targetMode !== "external" || isExternalProxyForwardType(t)).map((t) => (
            ...
            {user?.role === "admin" && (
              <div className="space-y-2 rounded-md border border-border/60 bg-muted/20 p-2.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button type="button" variant={form.targetMode === "manual" ? "default" : "outline"} onClick={() => setTargetMode("manual")}>手动目标</Button>
                  <Button type="button" variant={form.targetMode === "external" ? "default" : "outline"} disabled={externalProxyQuery.isLoading || externalProxyQuery.isError || !externalTargetRouteCompatible || (isForwardGroupRouteMode ? !isExternalProxyForwardType(effectiveRouteForwardType) : !usableForwardTypes.some(isExternalProxyForwardType))} onClick={() => setTargetMode("external")}>出口节点</Button>
                </div>
                {form.targetMode === "external" && <div className="space-y-2">
                  <Input type="search" aria-label="搜索规则出口节点" value={externalProxySearch} placeholder="搜索名称或地址" onChange={(event) => setExternalProxySearch(event.target.value)} />
                  <Select
                    value={form.targetExternalProxyNodeId ? String(form.targetExternalProxyNodeId) : undefined}
                    onValueChange={(value) => {
                      const node = externalProxyNodes.find((item) => Number(item.id) === Number(value));
                      if (!node) return;
                      setForm((prev) => ({ ...prev, targetExternalProxyNodeId: node.id, targetIp: node.address, targetPort: node.port, protocol: "tcp", proxyProtocolSend: false, proxyProtocolExitSend: false, failoverEnabled: false, failoverTargetsText: "" }));
                    }}
                  >
                    <SelectTrigger><SelectValue placeholder="请选择出口节点" /></SelectTrigger>
                    <SelectContent>{externalProxyNodes.map((node) => <SelectItem key={node.id} value={String(node.id)}>{node.name} · {node.protocol} · {node.address}:{node.port}</SelectItem>)}</SelectContent>
                  </Select>
                  <p className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs">六种本地转发工具只把 TCP 原始流量转发到该公开 endpoint，不解析代理协议；向上游发送 PROXY Protocol 的选项已清除。</p>
                </div>}
              </div>
            )}
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={form.targetIp}
                  readOnly={form.targetMode === "external"}
            ...
            {showMainBackupConfig && form.targetMode !== "external" && (
            <div className="space-y-2 rounded-md border border-border/60 bg-muted/20 p-2.5">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
      ...
      <Dialog open={!!managedRuleAction} onOpenChange={(open) => { if (!open) setManagedRuleAction(null); }}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>此规则由快速配置统一管理</DialogTitle>
            <DialogDescription>
              不能在普通规则页直接{managedRuleAction?.action === "edit" ? "编辑" : managedRuleAction?.action === "delete" ? "删除" : managedRuleAction?.rule?.isEnabled ? "停用" : "启用"}“{managedRuleAction?.rule?.name || "该规则"}”。
            </DialogDescription>
          </DialogHeader>
          {managedRuleAction && (
            <div className="space-y-3">
              <QuickConfigRuleRefView rule={managedRuleAction.rule} />
              <div role="note" className="flex items-start gap-2 rounded-md border border-amber-500/25 bg-amber-500/10 p-3 text-sm leading-6 text-amber-800 dark:text-amber-200">
                <AlertCircle className="mt-1 h-4 w-4 shrink-0" aria-hidden="true" />
                <p>
                  快速配置会一起维护运营商线路 DNS、入口 Realm 规则和共享端口。单独修改、停用或删除这条规则，可能让 DNS 继续指向不可用入口并造成拓扑状态不一致。请从快速配置中调整或删除整套拓扑。
                </p>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setManagedRuleAction(null)}>返回规则</Button>
            <Button
              type="button"
              onClick={() => {
                setManagedRuleAction(null);
                setLocation("/xray?tab=quick-config");
              }}
            >
              前往快速配置
            </Button>
