...
import { inspectQuickConfigPaths, quickConfigPathsFromEntries, QUICK_CONFIG_CARRIER_LABELS, type QuickConfigPaths } from "./xrayQuickConfigPaths";
...
export function XrayQuickConfigCarrierPaths(props: {
  ...
  return <div className="min-w-0 space-y-5">
    ...
    {!props.confirmedValid && <Alert variant="destructive"><AlertTitle>域名确认已过期</AlertTitle><AlertDescription>草稿保留，请返回域名重新检查。</AlertDescription></Alert>}
    ...
    <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-between">
      <Button type="button" variant="outline" className="h-11" onClick={props.onBack}>返回：域名</Button>
      <Button type="button" className="h-11" disabled={props.loading || props.error || !props.confirmedValid || !props.linesAvailable || inspection.issues.length > 0} onClick={props.onNext}>下一步：转发引擎</Button>
    </div>
