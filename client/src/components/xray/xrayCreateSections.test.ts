...
import { selectableXrayCreateSections } from "./XrayCreateSectionNav";
...
test("six-section create UI derives protocol, transport, and security axes only from available profiles", () => {
...
test("completed create sections remain selectable when a forward dependency expires", () => {
  ...
  assert.deepEqual([...selectable], ["BASIC", "PROTOCOL", "TRANSPORT", "SECURITY"]);
