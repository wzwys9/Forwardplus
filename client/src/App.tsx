...
import { resolveXrayUiAccess } from "@/lib/xrayNavigation";
...
function XrayRoute() {
  ...
  const access = resolveXrayUiAccess({
    authLoading: loading,
    userRole: user?.role,
    featureLoading: !!user && publicInfo.isLoading && !publicInfo.data,
    featureError: publicInfo.isError,
    featureEnabled: publicInfo.data?.xrayEnabled === true,
  });
  if (access === "WAIT") return null;
  if (access === "LOGIN") return <Redirect to="/login" />;
  if (access === "HOME") return <Redirect to="/" />;
  return <XrayPage />;
}
...
function Router() {
  return (
    <Switch>
      ...
      <Route path="/xray" component={XrayRoute} />
