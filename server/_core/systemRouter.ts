...
import {
  createXrayMasterKeyBackupBundleForSnapshot,
  prepareXrayMasterKeyBackupRestore,
} from "../xrayBackup";
...
export const REPO_URL = "https://github.com/wzwys9/Forwardplus";
...
const DEFAULT_DOCKER_IMAGE = "ghcr.io/wzwys9/forwardplus:latest";
...
type GithubReleaseInfo = {
  ...
  prerelease?: boolean;
  draft?: boolean;
  assets?: Array<{ id?: number; name?: string; state?: string; size?: number; browser_download_url?: string }>;
};
...
function githubContentsApiUrl(repoUrl: string, ref: string, path: string) {
  const { owner, repo } = githubRepoParts(repoUrl);
  const encodedPath = path.replace(/^\/+/, "").split("/").map(encodeURIComponent).join("/");
  return `${githubApiBase(repoUrl)}/contents/${encodedPath}?ref=${encodeURIComponent(ref)}`;
}
...
async function fetchGithubJson<T>(
  ...
  for (const candidate of candidates) {
    try {
      const requestUrl = noCacheUrl(candidate);
      const res = await fetch(requestUrl, withGithubAuth(requestUrl, {
        cache: "no-store",
        headers: {
          "Accept": "application/vnd.github+json",
          "Cache-Control": "no-cache",
          "Pragma": "no-cache",
          "User-Agent": `Forwardplus/${APP_VERSION}`,
        },
      }));
...
async function fetchGithubTextContent(
  ...
  const payload = await fetchGithubJson<{ content?: string; encoding?: string }>(
    githubContentsApiUrl(repoUrl, ref, path),
    accelerator,
  );
  if (payload.encoding !== "base64" || !payload.content) {
    throw new Error("GitHub Contents API 返回内容格式不正确");
  }
...
async function fetchPanelBundleAssetStatus(
  ...
  const ready = !!asset && asset.state === "uploaded" && Number(asset.size || 0) > 0;
  return { ready, status: ready ? 200 : 404, url };
}
...
async function fetchReleaseAgentVersion(
  ...
  for (const ref of refs) {
    try {
      const text = await fetchGithubTextContent(
        REPO_URL,
        ref,
        "shared/versions.ts",
        accelerator,
...
export const systemRouter = router({
  ...
  exportPanelBackup: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const xrayMasterKey = createXrayMasterKeyBackupBundleForSnapshot(backupSnapshot);
      const backup = encryptMigrationSnapshot(backupSnapshot, input.password, { xrayMasterKey });
  ...
  importPanelBackup: adminProcedure
    ...
    .mutation(async ({ input }) => withKeyedTaskLock("panel-backup-import", async () => {
      ...
      const decrypted = decryptPanelBackup(input.content, input.password);
      ...
      const keyRestore = await prepareXrayMasterKeyBackupRestore(snapshot, decrypted.xrayMasterKey);
  ...
  /** 由面板代查私有 GitHub 仓库中的 Android 安装包 */
  mobileAppUpdate: protectedProcedure.query(async () => {
    const releases = await fetchGithubJson<GithubReleaseInfo[]>(
      `${githubApiBase(REPO_URL)}/releases?per_page=20`,
      await readPanelUpdateAccelerator(),
    );
    const candidates = releases
      .filter((release) => !release.draft && !release.prerelease)
      .flatMap((release) => (release.assets || []).flatMap((asset) => {
        const match = String(asset.name || "").match(/^forwardx-android-v?(\d+\.\d+\.\d+)(?:-[\w.-]+)?\.apk$/i);
        if (!match) return [];
        return [{
    ...
    return {
      ...
      releaseUrl: candidates[0]?.releaseUrl || `${REPO_URL}/releases/latest`,
      downloadUrl: candidates[0]?.assetId ? mobileReleaseDownloadPath(candidates[0].assetId) : null,
      hasPackage: candidates.length > 0,
    };
  }),
