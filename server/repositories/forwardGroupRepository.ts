...
export async function getForwardGroups(userId?: number, options: { includeRuntime?: boolean; ids?: number[] } = {}) {
  ...
  const quickConfigRules = await db
    .select({
      id: forwardRules.id,
      portResourceGroupId: forwardRules.portResourceGroupId,
    })
    .from(forwardRules)
    .where(and(
      inArray(forwardRules.portResourceGroupId, ids),
      isNotNull(forwardRules.xrayQuickConfigId),
      sql`${forwardRules.xrayQuickConfigId} > 0`,
      eq(forwardRules.pendingDelete, false),
    ));
  ...
  const quickConfigCountByGroup = new Map<number, number>();
  for (const rule of quickConfigRules as any[]) {
    const groupId = Number(rule.portResourceGroupId || 0);
    if (groupId > 0) quickConfigCountByGroup.set(groupId, (quickConfigCountByGroup.get(groupId) || 0) + 1);
  }
  ...
  return groupRows.map((group: any) => {
    ...
    const runtime = options.includeRuntime
      ...
      : null;
    const templateRuleCount = templateCountByGroup.get(groupId) || 0;
    const quickConfigRuleCount = quickConfigCountByGroup.get(groupId) || 0;
    return {
      ...group,
      systemManagedKey: undefined,
      groupMode: groupModeOf(group),
      templateRuleCount,
      quickConfigRuleCount,
      referenceRuleCount: templateRuleCount + quickConfigRuleCount,
      quickConfigLocked: quickConfigRuleCount > 0,
      ...(runtime ? {
...
export async function getForwardGroupById(id: number) {
  ...
  return { ...group, systemManagedKey: undefined, groupMode: groupModeOf(group), members: hydratedMembers };
}

export async function getQuickConfigPortResourceRuleCount(groupId: number) {
  const q = quoteIdentifier;
  const [row] = await queryRaw<{ count: number }>(
    `SELECT COUNT(*) AS ${q("count")} FROM ${q("forward_rules")}
      WHERE ${q("portResourceGroupId")} = ? AND ${q("xrayQuickConfigId")} IS NOT NULL
        AND ${q("xrayQuickConfigId")} > 0 AND ${q("pendingDelete")} = ?`,
    [groupId, boolValue(false)],
  );
  return Number(row?.count || 0);
}

function quickConfigPortResourceInUseError() {
  return new Error("QUICK_CONFIG_PORT_RESOURCE_IN_USE");
}

async function assertQuickConfigPortResourceNotInUse(groupId: number) {
  if (await getQuickConfigPortResourceRuleCount(groupId) > 0) throw quickConfigPortResourceInUseError();
}

function forwardGroupUpdateValuesEqual(current: unknown, next: unknown) {
  if (current === next) return true;
  if ((current === null || current === undefined) && (next === null || next === undefined)) return true;
  if (typeof current === "boolean" || typeof next === "boolean") return !!current === !!next;
...
export function filterForwardGroupFieldsForUse(
  ...
  return groups.map((group: any) => ({
    ...
    quickConfigRuleCount: group.quickConfigRuleCount,
    referenceRuleCount: group.referenceRuleCount,
    quickConfigLocked: !!group.quickConfigLocked,
    systemManagedKind: group.systemManagedKind || null,
...
export async function setForwardGroupEnabled(groupId: number, isEnabled: boolean) {
  ...
  if (!isEnabled) await assertQuickConfigPortResourceNotInUse(groupId);
  const mode = groupModeOf(group);
...
async function ensureMemberRuleForTemplate(group: any, templateRule: any, member: any, options: SyncForwardGroupRulesOptions = {}) {
  ...
  const hasExternalProxyTarget = Number((templateRule as any).targetExternalProxyNodeId || 0) > 0;
  ...
  const payload: any = {
    ...
    targetExternalProxyNodeId: (templateRule as any).targetExternalProxyNodeId ?? null,
    ...
    proxyProtocolSend: hasExternalProxyTarget ? false : member.memberType === "tunnel" ? tunnelProxySupported && protocolTcpSupported && !!tunnel.proxyProtocolSend : directProxySupported && !!(directRuntimeSource as any).proxyProtocolSend,
    ...
    proxyProtocolExitSend: hasExternalProxyTarget ? false : member.memberType === "tunnel" ? tunnelProxySupported && protocolTcpSupported && !!tunnel.proxyProtocolExitSend : false,
...
export async function updateForwardGroup(id: number, data: Partial<InsertForwardGroup>, options: { skipSync?: boolean } = {}) {
  const db = await getDb();
  const quickConfigReferences = await getQuickConfigPortResourceRuleCount(id);
  if (quickConfigReferences > 0) {
    const existing = await getForwardGroupById(id) as any;
    const displayOnlyFields = new Set(["name", "remark", "sortOrder", "updatedAt"]);
    const changesProtectedField = Object.entries(data as Record<string, unknown>).some(([field, value]) => (
      value !== undefined
      && !displayOnlyFields.has(field)
      && !forwardGroupUpdateValuesEqual(existing?.[field], value)
    ));
    if (changesProtectedField) throw quickConfigPortResourceInUseError();
  }
...
export async function replaceForwardGroupMembers(
  ...
  if (members.length === 0) throw new Error("转发组至少需要一个成员");
  await assertQuickConfigPortResourceNotInUse(groupId);
  const group = await getForwardGroupById(groupId);
  const groupMode = groupModeOf(group);
...
export async function deleteForwardGroup(id: number) {
  await assertQuickConfigPortResourceNotInUse(id);
  const db = await getDb();
