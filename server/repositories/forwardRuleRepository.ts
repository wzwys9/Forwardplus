...
import { executeRaw, getDb, insertAndGetId, nowDate, withDatabaseTransaction } from "../dbRuntime";
...
import { buildGlobalPortReferenceKey } from "../globalPortBackfill";
import {
...
function forwardRuleGlobalNetwork(value: unknown): "TCP" | "UDP" | "BOTH" {
...
function positiveRuleId(value: unknown): number {
  const id = Number(value);
  if (!Number.isSafeInteger(id) || id <= 0) throw new Error("GLOBAL_PORT_INVALID");
  return id;
}

function ordinaryForwardRuleOwnerId(rule: any): number | null {
  if (rule?.isForwardGroupTemplate || rule?.xrayQuickConfigId !== null && rule?.xrayQuickConfigId !== undefined) {
    return null;
  }
  return positiveRuleId(rule?.forwardGroupRuleId ?? rule?.id);
}

function forwardRulePortReference(input: {
  rule: any;
  role: "PUBLIC_LISTENER" | "OWNERSHIP";
  resourceType?: "FORWARD_RULE" | "FORWARD_RULE_TUNNEL_EXIT";
  resourceId?: number;
  hostId?: number;
  portSlot?: "PRIMARY" | "MAPPED";
}): GlobalPortReferenceInput {
  return {
    resourceType: input.resourceType ?? "FORWARD_RULE",
    resourceId: positiveRuleId(input.resourceId ?? input.rule.id),
    hostId: positiveRuleId(input.hostId ?? input.rule.hostId),
    network: forwardRuleGlobalNetwork(input.rule.protocol),
    role: input.role,
    isOwning: true,
    ...(input.portSlot ? { slot: input.portSlot } : {}),
  };
}

async function acquireForwardRulePortReference(input: {
  port: number;
  ownerId: number;
  reference: GlobalPortReferenceInput;
}) {
  const owner = { type: "FORWARD_RULE" as const, stableIdentity: input.ownerId };
  const current = await inspectGlobalPortAllocation(input.port);
  ...
  const reservation = await reserveGlobalPortAllocation({ port: input.port, owner });
...
async function stageForwardRuleSourcePort(rule: any, nextPort: number) {
  const ownerId = ordinaryForwardRuleOwnerId(rule);
  if (!ownerId) return;
  await acquireForwardRulePortReference({
    port: nextPort,
    ownerId,
    reference: forwardRulePortReference({ rule, role: "OWNERSHIP" }),
  });
}

async function reconcileForwardRuleSourcePort(rule: any) {
  const ownerId = ordinaryForwardRuleOwnerId(rule);
  if (!ownerId) return;
  const stagedReference = forwardRulePortReference({ rule, role: "OWNERSHIP" });
  const staged = await inspectGlobalPortReferenceAllocation(stagedReference);
  if (!staged) return;
  const sourcePort = positiveRuleId(rule.sourcePort);
  if (staged.port !== sourcePort) throw new Error("GLOBAL_PORT_CONFLICT");
  const publicReference = await findForwardRulePublicReference(positiveRuleId(rule.id));
  if (!publicReference) throw new Error("GLOBAL_PORT_CONFLICT");
  const nextPublicReference = forwardRulePortReference({ rule, role: "PUBLIC_LISTENER" });
  await promoteStagedGlobalPortOwningReference({
    owner: { type: "FORWARD_RULE", stableIdentity: ownerId },
    publicReference,
    nextPublicReference,
    stagedReference,
  });
...
async function findForwardRulePublicReference(ruleId: number): Promise<GlobalPortReferenceInput | null> {
  const q = quoteIdentifier;
  const rows = await queryRaw<Record<string, unknown>>(
    `SELECT ${["referenceKey", "resourceId", "hostId", "network", "role", "isOwning"].map(q).join(", ")}
       FROM ${q("global_port_allocation_references")}
      WHERE ${q("resourceType")} = 'FORWARD_RULE' AND ${q("resourceId")} = ?
        AND ${q("role")} = 'PUBLIC_LISTENER' ORDER BY ${q("id")} ASC`,
    [ruleId],
  );
  for (const row of rows) {
    const network = String(row.network);
    if (network !== "TCP" && network !== "UDP" && network !== "BOTH") continue;
    const reference: GlobalPortReferenceInput = {
      resourceType: "FORWARD_RULE",
      resourceId: ruleId,
      hostId: positiveRuleId(row.hostId),
      network,
      role: "PUBLIC_LISTENER",
      isOwning: row.isOwning === true || row.isOwning === 1 || row.isOwning === "1",
    };
    if (reference.isOwning && String(row.referenceKey) === buildGlobalPortReferenceKey(reference)) return reference;
  }
  return null;
}
...
export type ForwardRuleListQuery = PageRequest & {
  ...
  search?: string;
  /** Link-management projection for local root rules not represented by a saved resource. */
};
...
export async function createForwardRule(rule: InsertForwardRule) {
  ...
  return withDatabaseTransaction(async () => {
    ...
    const ownerId = ordinaryForwardRuleOwnerId(created);
    if (ownerId && created) {
      await acquireForwardRulePortReference({
        port: positiveRuleId((created as any).sourcePort),
        ownerId,
        reference: forwardRulePortReference({ rule: created, role: "PUBLIC_LISTENER" }),
      });
    }
...
export async function updateForwardRule(id: number, data: Partial<InsertForwardRule>) {
  ...
  await withDatabaseTransaction(async () => {
    ...
    const changesListenerIdentity = data.sourcePort !== undefined || data.hostId !== undefined || data.protocol !== undefined;
    const before = audit || changesListenerIdentity ? await getForwardRuleById(id).catch(() => undefined) : undefined;
    await txDb.update(forwardRules).set({ ...data, updatedAt: nowDate() }).where(eq(forwardRules.id, id));
    const after = audit || changesListenerIdentity ? await getForwardRuleById(id).catch(() => undefined) : undefined;
    if (changesListenerIdentity && before && after
      && (Number((before as any).sourcePort) !== Number((after as any).sourcePort)
        || Number((before as any).hostId) !== Number((after as any).hostId)
        || String((before as any).protocol) !== String((after as any).protocol))) {
      await stageForwardRuleSourcePort(after, positiveRuleId((after as any).sourcePort));
    }
    if (audit && before) {
...
async function releaseForwardRulePortsAfterRuntimeCleanup(rule: any) {
  if (!ordinaryForwardRuleOwnerId(rule)) return;
  const publicReference = await findForwardRulePublicReference(positiveRuleId(rule.id));
  if (publicReference) {
  ...
  const tunnelExitPort = Number(rule?.tunnelExitPort ?? 0);
  if (tunnelId > 0 && tunnelExitPort > 0) {
    const q = quoteIdentifier;
    const exits = await queryRaw<{ exitHostId: unknown }>(
      `SELECT ${q("exitHostId")} FROM ${q("tunnels")} WHERE ${q("id")} = ? LIMIT 1`,
      [tunnelId],
    );
    const exitHostId = Number(exits[0]?.exitHostId ?? 0);
    if (exitHostId > 0) {
      await releaseGlobalPortReferenceAfterRuntimeCleanup({
        reference: forwardRulePortReference({
          rule,
          role: "PUBLIC_LISTENER",
          hostId: exitHostId,
          portSlot: "PRIMARY",
  ...
  const q = quoteIdentifier;
  const mappedExits = await queryRaw<Record<string, unknown>>(
    `SELECT ${["id", "exitHostId"].map(q).join(", ")}
       FROM ${q("forward_rule_tunnel_exits")} WHERE ${q("ruleId")} = ? ORDER BY ${q("id")} ASC`,
    [positiveRuleId(rule.id)],
  );
  for (const exit of mappedExits) {
    await releaseGlobalPortReferenceAfterRuntimeCleanup({
      reference: forwardRulePortReference({
        rule,
        role: "PUBLIC_LISTENER",
        resourceType: "FORWARD_RULE_TUNNEL_EXIT",
        resourceId: positiveRuleId(exit.id),
        hostId: positiveRuleId(exit.exitHostId),
        portSlot: "MAPPED",
      }),
    });
  }
...
export async function finalizeForwardRuleDelete(id: number) {
  ...
  await withDatabaseTransaction(async () => {
    ...
    if (!rule || !(rule as any).pendingDelete) return;
    await releaseForwardRulePortsAfterRuntimeCleanup(rule);
    await txDb.delete(forwardRuleTunnelExits).where(eq(forwardRuleTunnelExits.ruleId, id));
    await txDb.update(forwardGroupMembers).set({
      ruleId: null,
      updatedAt: nowDate(),
    } as any).where(eq(forwardGroupMembers.ruleId, id));
    await txDb.delete(forwardRules).where(and(
...
export async function updateRuleRunningStatus(id: number, isRunning: boolean) {
  ...
  if (rule && isRunning) await reconcileForwardRuleSourcePort(rule);
  await db.update(forwardRules).set({ isRunning, updatedAt: nowDate() }).where(eq(forwardRules.id, id));
  if (Number((rule as any)?.xrayQuickConfigId || 0) > 0) {
