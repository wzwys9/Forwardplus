...
import {
  assertMigrationSnapshotXraySecretsAvailable,
  type XrayMasterKeyBackupBundle,
} from "./xrayBackup";
import { XraySecretUnavailableError } from "./xraySecretCrypto";
...
const XRAY_GENERIC_SECRET_TABLE_NAMES = [
  "xray_access_entries",
  "xray_access_secrets",
...
const DNS_PROVIDER_TABLE_NAMES = [
...
const QUICK_CONFIG_TABLE_NAMES = [
  "xray_quick_configs",
...
const DIRECT_SQLITE_OPTIONAL_SOURCE_TABLES = new Set([
  "agent_traffic_reports",
  "xray_inbounds",
  "xray_clients",
  ...XRAY_GENERIC_SECRET_TABLE_NAMES,
  ...
  ...DNS_PROVIDER_TABLE_NAMES,
  ...QUICK_CONFIG_TABLE_NAMES,
]);
...
const GLOBAL_PORT_RESOURCE_TABLES: Record<string, string> = {
  XRAY_INBOUND: "xray_inbounds",
  FORWARD_RULE: "forward_rules",
  MANAGED_SERVICE: "xray_managed_services",
  TUNNEL: "tunnels",
  TUNNEL_EXIT_NODE: "tunnel_exit_nodes",
  TUNNEL_HOP: "tunnel_hops",
  FORWARD_RULE_TUNNEL_EXIT: "forward_rule_tunnel_exits",
  QUICK_CONFIG: "xray_quick_configs",
};

function mapGlobalPortResourceId(maps: ImportMaps, resourceType: unknown, resourceId: unknown) {
  const table = GLOBAL_PORT_RESOURCE_TABLES[String(resourceType)];
  if (!table) throw new Error(`全局端口引用类型无效：${String(resourceType)}`);
  return mapRequiredId(maps, table, resourceId);
}

function remapGlobalPortOwnerGroupTag(maps: ImportMaps, value: unknown) {
  const tag = String(value ?? "");
  const match = /^(forward-rule|tunnel):(\d+)$/.exec(tag);
  if (!match) return tag;
  const table = ({
    "forward-rule": "forward_rules",
    tunnel: "tunnels",
  } as Record<string, string>)[match[1]];
  const ownerType = match[1] === "forward-rule" ? "FORWARD_RULE" : "TUNNEL";
  const mappedId = mapRequiredId(maps, table, match[2]);
  return buildGlobalPortOwnerGroupTag(ownerType, mappedId);
}
...
const PANEL_DATA_EXTRA_BUSINESS_TABLES = MIGRATION_TABLES.filter(
  (table) => table !== "system_settings"
    && table !== "dns_provider_global_bindings"
    && table !== "global_port_scan_leases"
    && !(PANEL_DATA_SUMMARY_TABLES as readonly string[]).includes(table),
);
...
async function hasExtraBusinessData() {
  if (PANEL_DATA_EXTRA_BUSINESS_TABLES.length > 0) {
    ...
    if (rows.length > 0 && rows.some((row) => row.has_rows === true || Number(row.has_rows || 0) > 0)) {
      return true;
    }
    if (rows.length === 0) {
  ...
  const bindingRows = await queryRaw(
    `SELECT 1 AS present FROM ${quote("dns_provider_global_bindings")} WHERE ${quote("scopeKey")} <> ? OR ${quote("accountId")} IS NOT NULL OR ${quote("revision")} <> 1 LIMIT 1`,
    ["XRAY_QUICK_CONFIG"],
  ).catch(() => []);
  if (bindingRows.length > 0) return true;
  const scanLeaseRows = await queryRaw(
    `SELECT 1 AS present FROM ${quote("global_port_scan_leases")} WHERE ${quote("scopeKey")} <> ? OR ${quote("leaseOwnerHash")} IS NOT NULL OR ${quote("leaseUntil")} IS NOT NULL LIMIT 1`,
    ["GLOBAL_PORT_RECLAIM"],
  ).catch(() => []);
  return scanLeaseRows.length > 0;
}
...
type DecryptedPanelBackup = {
  snapshot: MigrationSnapshot;
  xrayMasterKey?: XrayMasterKeyBackupBundle;
};

function backupAAD(backup: Omit<EncryptedPanelBackup, "tag" | "data">) {
  return Buffer.from(JSON.stringify(backup), "utf8");
}

function exactBackupKdf(value: EncryptedPanelBackup["kdf"] | undefined) {
  return value?.name === BACKUP_KDF.name
    && value.keyLength === BACKUP_KDF.keyLength
    && value.cost === BACKUP_KDF.cost
    && value.blockSize === BACKUP_KDF.blockSize
    && value.parallelization === BACKUP_KDF.parallelization;
}

function decodeBackupBase64(value: unknown, expectedBytes?: number) {
...
export function encryptMigrationSnapshot(
  snapshot: MigrationSnapshot,
  password: string,
  options: { xrayMasterKey?: XrayMasterKeyBackupBundle } = {},
): EncryptedPanelBackup {
  ...
  const header: Omit<EncryptedPanelBackup, "tag" | "data"> = {
    ...
    kdf: { ...BACKUP_KDF, salt: salt.toString("base64") },
    iv: iv.toString("base64"),
  };
  const cipher = nodeCrypto.createCipheriv(BACKUP_CIPHER, key, iv);
  cipher.setAAD(backupAAD(header));
  const payload = Buffer.from(JSON.stringify({
    format: "forwardx-panel-backup-payload",
    version: 2,
    snapshot,
    ...(options.xrayMasterKey ? { xrayMasterKey: options.xrayMasterKey } : {}),
  }), "utf8");
...
export function decryptPanelBackup(rawContent: string, password: string): DecryptedPanelBackup {
  ...
  if (backup?.format !== BACKUP_FORMAT || (backup.version !== 1 && backup.version !== 2) || !backup.encrypted) {
    throw new Error("备份文件格式无效或版本不受支持");
  }
  if (backup.cipher !== BACKUP_CIPHER || !exactBackupKdf(backup.kdf)) {
    throw new Error("备份文件加密参数无效");
  }
  const salt = decodeBackupBase64(backup.kdf.salt, 16);
  const iv = decodeBackupBase64(backup.iv, 12);
  const tag = decodeBackupBase64(backup.tag, 16);
  const data = decodeBackupBase64(backup.data);
  if (data.length < 1) throw new Error("备份文件加密参数无效");
  try {
    const key = deriveBackupKey(String(password || ""), salt);
    const decipher = nodeCrypto.createDecipheriv(BACKUP_CIPHER, key, iv);
    if (backup.version === 2) {
      const { tag: _tag, data: _data, ...header } = backup;
    ...
    const snapshot = backup.version === 1 ? parsed as MigrationSnapshot : parsed?.snapshot as MigrationSnapshot;
    if (!snapshot || snapshot.version !== 1 || !snapshot.tables || typeof snapshot.tables !== "object") {
      throw new Error("备份内容无效");
    }
    if (backup.version === 2 && (parsed?.format !== "forwardx-panel-backup-payload" || parsed?.version !== 2)) {
      throw new Error("备份内容无效");
    }
    return {
      snapshot,
      ...(backup.version === 2 && parsed.xrayMasterKey !== undefined ? { xrayMasterKey: parsed.xrayMasterKey } : {}),
    };
  } catch (error) {
...
export function decryptMigrationSnapshotBackup(rawContent: string, password: string): MigrationSnapshot {
  return decryptPanelBackup(rawContent, password).snapshot;
}
...
type DnsProviderBindingImportPlan = Readonly<{
  sourceId: number;
  targetId: number;
  targetRevision: number;
  action: "restore-seed" | "bind-empty" | "preserve";
}>;

async function planDnsProviderBindingImport(
  snapshot: MigrationSnapshot,
  mode: "restore" | "incremental",
): Promise<DnsProviderBindingImportPlan | null> {
  const sourceRows = sortedSnapshotRows(snapshot, "dns_provider_global_bindings");
  if (sourceRows.length === 0) return null;
  const source = sourceRows[0];
  const sourceId = toNumberId(source.id);
  if (!sourceId || sourceRows.length !== 1 || source.scopeKey !== "XRAY_QUICK_CONFIG") {
    throw new XraySecretUnavailableError();
  }
  const targetRows = await queryRaw<Record<string, any>>(
    `SELECT ${quote("id")}, ${quote("scopeKey")}, ${quote("accountId")}, ${quote("revision")} FROM ${quote("dns_provider_global_bindings")}`,
  );
  const target = targetRows[0];
  const targetId = toNumberId(target?.id);
  const targetRevision = Number(target?.revision);
  if (targetRows.length !== 1 || !targetId || target.scopeKey !== "XRAY_QUICK_CONFIG") {
    throw new XraySecretUnavailableError();
  }
  if (!Number.isSafeInteger(targetRevision) || targetRevision <= 0) throw new XraySecretUnavailableError();
  if (target.accountId === null || target.accountId === undefined) {
    if (mode === "restore") {
      if (targetRevision !== 1) throw new XraySecretUnavailableError();
      return { sourceId, targetId, targetRevision, action: "restore-seed" };
    }
    if (source.accountId === null || source.accountId === undefined) {
      return { sourceId, targetId, targetRevision, action: "preserve" };
    }
    if (targetRevision >= Number.MAX_SAFE_INTEGER) throw new XraySecretUnavailableError();
    return { sourceId, targetId, targetRevision, action: "bind-empty" };
  }
  if (mode !== "incremental") throw new XraySecretUnavailableError();
  if (source.accountId === null || source.accountId === undefined) {
    throw new XraySecretUnavailableError();
  }
  const sourceAccountId = toNumberId(source.accountId);
  const sourceAccount = (snapshot.tables?.dns_provider_accounts || [])
    .find((row) => toNumberId(row.id) === sourceAccountId);
  const targetAccountId = toNumberId(target.accountId);
  const [targetAccount] = targetAccountId
    ? await queryRaw<Record<string, any>>(
      `SELECT ${quote("accountTag")} FROM ${quote("dns_provider_accounts")} WHERE ${quote("id")} = ? LIMIT 1`,
      [targetAccountId],
    )
    : [];
  if (!sourceAccount || !targetAccount || sourceAccount.accountTag !== targetAccount.accountTag) {
    throw new XraySecretUnavailableError();
  }
  return { sourceId, targetId, targetRevision, action: "preserve" };
}

function changedRowCount(result: any): number {
...
async function insertRestoreRowsStrict(
  table: string,
  rows: PreparedRestoreRow[],
  onInserted: (row: PreparedRestoreRow) => void,
) {
  ...
  for (const group of groups.values()) {
    ...
    for (let index = 0; index < group.length; index += rowsPerBatch) {
      const batch = group.slice(index, index + rowsPerBatch);
      await executeRestoreBatch(table, columns, batch);
      for (const row of batch) onInserted(row);
    }
  }
}
...
async function insertGeneratedRowsStrict(
  table: string,
  rows: PreparedGeneratedRow[],
  mysqlStep: number,
  onInserted: (row: PreparedGeneratedRow, id: number) => void,
  ...
  for (const group of groups.values()) {
    ...
    for (let index = 0; index < group.length; index += rowsPerBatch) {
      const batch = group.slice(index, index + rowsPerBatch);
      const ids = await executeGeneratedBatch(table, columns, batch, mysqlStep);
      batch.forEach((row, batchIndex) => onInserted(row, ids[batchIndex]));
    }
  }
}
...
async function prepareImportRow(table: string, source: Record<string, any>, maps: ImportMaps): Promise<PreparedImportRow | null> {
  ...
  switch (table) {
    ...
    case "xray_tls_certificates":
      row.hostId = mapRequiredId(maps, "hosts", source.hostId);
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      return { row, existingWhere: row.certificateTag ? { certificateTag: row.certificateTag } : undefined };

    case "xray_external_proxy_nodes":
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      return { row, existingWhere: row.nodeTag ? { nodeTag: row.nodeTag } : undefined };

    case "xray_external_proxy_secrets":
      row.externalProxyNodeId = mapRequiredId(maps, "xray_external_proxy_nodes", source.externalProxyNodeId);
      return { row, existingWhere: { externalProxyNodeId: row.externalProxyNodeId, kind: row.kind } };

    case "dns_provider_accounts":
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      return { row, existingWhere: row.accountTag ? { accountTag: row.accountTag } : undefined };

    case "dns_provider_account_secrets":
      row.accountId = mapRequiredId(maps, "dns_provider_accounts", source.accountId);
      return { row, existingWhere: { accountId: row.accountId, kind: row.kind } };

    case "dns_provider_global_bindings":
      row.accountId = mapOptionalId(maps, "dns_provider_accounts", source.accountId);
      return { row, existingWhere: { scopeKey: row.scopeKey } };

    case "dns_provider_zones":
      row.accountId = mapRequiredId(maps, "dns_provider_accounts", source.accountId);
      return { row, existingWhere: { accountId: row.accountId, providerZoneId: row.providerZoneId } };

    case "dns_provider_record_lines":
      row.zoneId = mapRequiredId(maps, "dns_provider_zones", source.zoneId);
      return { row, existingWhere: { zoneId: row.zoneId, providerLineId: row.providerLineId } };

    case "global_port_allocations":
      return { row, existingWhere: row.allocationTag ? { allocationTag: row.allocationTag } : undefined };

    case "xray_inbounds":
      row.hostId = mapRequiredId(maps, "hosts", source.hostId);
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      row.tlsCertificateId = mapOptionalId(maps, "xray_tls_certificates", source.tlsCertificateId);
      row.externalProxyNodeId = mapOptionalId(maps, "xray_external_proxy_nodes", source.externalProxyNodeId);
      return { row, existingWhere: row.runtimeTag ? { runtimeTag: row.runtimeTag } : undefined };

    case "xray_quick_configs":
      row.xrayInboundId = mapOptionalId(maps, "xray_inbounds", source.xrayInboundId);
      row.externalProxyNodeId = mapOptionalId(maps, "xray_external_proxy_nodes", source.externalProxyNodeId);
      row.dnsAccountId = mapRequiredId(maps, "dns_provider_accounts", source.dnsAccountId);
      row.zoneId = mapRequiredId(maps, "dns_provider_zones", source.zoneId);
      row.activeTopologyRevisionId = null;
      row.desiredTopologyRevisionId = null;
      row.currentOperationId = null;
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      return { row, existingWhere: row.configTag ? { configTag: row.configTag } : undefined };

    case "xray_quick_config_domain_claims":
      row.dnsAccountId = mapRequiredId(maps, "dns_provider_accounts", source.dnsAccountId);
      row.zoneId = mapRequiredId(maps, "dns_provider_zones", source.zoneId);
      row.quickConfigId = mapRequiredId(maps, "xray_quick_configs", source.quickConfigId);
      return { row, existingWhere: row.claimKey ? { claimKey: row.claimKey } : undefined };

    case "xray_quick_config_topology_revisions":
      row.quickConfigId = mapRequiredId(maps, "xray_quick_configs", source.quickConfigId);
      row.portAllocationId = mapRequiredId(maps, "global_port_allocations", source.portAllocationId);
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      return { row, existingWhere: { quickConfigId: row.quickConfigId, revisionNumber: row.revisionNumber } };

    case "xray_quick_config_routes":
      row.quickConfigId = mapRequiredId(maps, "xray_quick_configs", source.quickConfigId);
    ...
    case "xray_quick_config_operations":
      row.quickConfigId = mapRequiredId(maps, "xray_quick_configs", source.quickConfigId);
      row.fromTopologyRevisionId = mapOptionalId(maps, "xray_quick_config_topology_revisions", source.fromTopologyRevisionId);
      row.toTopologyRevisionId = mapOptionalId(maps, "xray_quick_config_topology_revisions", source.toTopologyRevisionId);
      row.retryOfOperationId = null;
      row.executionOwnerId = null;
      row.executionLeaseUntil = null;
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      return { row, existingWhere: row.operationTag ? { operationTag: row.operationTag } : undefined };

    case "xray_quick_config_operation_steps":
      row.operationId = mapRequiredId(maps, "xray_quick_config_operations", source.operationId);
      if (source.subjectId !== null && source.subjectId !== undefined) {
        const subjectTable = ({
          RULE: "forward_rules",
          DNS_RECORD: "xray_quick_config_dns_records",
          ALLOCATION: "global_port_allocations",
          TOPOLOGY: "xray_quick_config_topology_revisions",
        } as Record<string, string>)[String(source.subjectType)];
        if (subjectTable) row.subjectId = String(mapRequiredId(maps, subjectTable, source.subjectId));
      }
      ...
      row.startedAt = null;
      return { row, existingWhere: row.idempotencyKey ? { idempotencyKey: row.idempotencyKey } : undefined };

    case "xray_quick_config_dns_records":
      row.quickConfigId = mapRequiredId(maps, "xray_quick_configs", source.quickConfigId);
      row.routeId = mapRequiredId(maps, "xray_quick_config_routes", source.routeId);
      row.dnsAccountId = mapRequiredId(maps, "dns_provider_accounts", source.dnsAccountId);
      row.zoneId = mapRequiredId(maps, "dns_provider_zones", source.zoneId);
      return { row, existingWhere: row.recordTag ? { recordTag: row.recordTag } : undefined };

    case "xray_quick_config_dns_record_backups":
      row.operationId = mapRequiredId(maps, "xray_quick_config_operations", source.operationId);
      row.dnsAccountId = mapRequiredId(maps, "dns_provider_accounts", source.dnsAccountId);
      row.zoneId = mapRequiredId(maps, "dns_provider_zones", source.zoneId);
      return {
        ...
        existingWhere: {
          ...
          dnsAccountId: row.dnsAccountId,
          providerRecordId: row.providerRecordId,
        },
      };

    case "xray_clients":
      row.inboundId = mapRequiredId(maps, "xray_inbounds", source.inboundId);
      row.ownerUserId = mapOptionalId(maps, "users", source.ownerUserId);
      return { row, existingWhere: row.uuidFingerprint ? { uuidFingerprint: row.uuidFingerprint } : undefined };

    case "xray_access_entries":
      row.inboundId = mapRequiredId(maps, "xray_inbounds", source.inboundId);
      row.legacyClientId = mapOptionalId(maps, "xray_clients", source.legacyClientId);
      row.ownerUserId = mapOptionalId(maps, "users", source.ownerUserId);
      return {
        row,
        existingWhere: row.legacyClientId
          ? { legacyClientId: row.legacyClientId }
          : { statsKey: row.statsKey },
      };

    case "xray_access_secrets":
      row.accessEntryId = mapRequiredId(maps, "xray_access_entries", source.accessEntryId);
      return { row, existingWhere: { accessEntryId: row.accessEntryId, kind: row.kind } };

    case "xray_inbound_secrets":
      row.inboundId = mapRequiredId(maps, "xray_inbounds", source.inboundId);
      return { row, existingWhere: { inboundId: row.inboundId, kind: row.kind } };

    case "xray_artifacts":
      return {
        row,
        existingWhere: row.version && row.os && row.arch
          ? { version: row.version, os: row.os, arch: row.arch }
          : undefined,
      };

    case "xray_operations":
      row.hostId = mapRequiredId(maps, "hosts", source.hostId);
      row.inboundId = mapOptionalId(maps, "xray_inbounds", source.inboundId);
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      return { row, existingWhere: row.operationId ? { operationId: row.operationId } : undefined };

    case "xray_host_deployments":
      row.hostId = mapRequiredId(maps, "hosts", source.hostId);
      return { row, existingWhere: { hostId: row.hostId } };

    case "xray_runtime_reports":
      row.hostId = mapRequiredId(maps, "hosts", source.hostId);
      return { row, existingWhere: { hostId: row.hostId } };

    case "xray_managed_services":
      row.hostId = mapRequiredId(maps, "hosts", source.hostId);
      row.createdByUserId = mapRequiredId(maps, "users", source.createdByUserId);
      return { row, existingWhere: row.serviceTag ? { serviceTag: row.serviceTag } : undefined };

    case "xray_managed_service_instance_secrets":
      row.serviceId = mapRequiredId(maps, "xray_managed_services", source.serviceId);
      return { row, existingWhere: { serviceId: row.serviceId, kind: row.kind } };

    case "xray_managed_service_accounts":
      row.serviceId = mapRequiredId(maps, "xray_managed_services", source.serviceId);
      return { row, existingWhere: row.accountTag ? { accountTag: row.accountTag } : undefined };

    case "xray_managed_service_secrets":
      row.accountId = mapRequiredId(maps, "xray_managed_service_accounts", source.accountId);
      return { row, existingWhere: { accountId: row.accountId, kind: row.kind } };

    case "xray_managed_service_artifacts":
      return {
        row,
        existingWhere: row.kind && row.version && row.os && row.arch
          ? { kind: row.kind, version: row.version, os: row.os, arch: row.arch }
          : undefined,
      };

    case "xray_managed_service_deployments":
    case "xray_managed_service_runtime_reports":
      row.hostId = mapRequiredId(maps, "hosts", source.hostId);
      return { row, existingWhere: { hostId: row.hostId } };
    ...
    case "forward_rules":
      ...
      row.targetExternalProxyNodeId = mapOptionalId(maps, "xray_external_proxy_nodes", source.targetExternalProxyNodeId);
      row.xrayQuickConfigId = mapOptionalId(maps, "xray_quick_configs", source.xrayQuickConfigId);
      row.isRunning = false;
      row.pendingDelete = source.pendingDelete ?? false;
      return {
        row,
        existingWhere: row.xrayQuickConfigId ? {
          xrayQuickConfigId: row.xrayQuickConfigId,
          hostId: row.hostId,
    ...
    case "xray_quick_config_rule_bindings":
      row.quickConfigId = mapRequiredId(maps, "xray_quick_configs", source.quickConfigId);
      row.topologyRevisionId = mapRequiredId(maps, "xray_quick_config_topology_revisions", source.topologyRevisionId);
      row.forwardRuleId = mapRequiredId(maps, "forward_rules", source.forwardRuleId);
      return { row, existingWhere: row.bindingTag ? { bindingTag: row.bindingTag } : undefined };
    ...
    case "global_port_allocation_references":
      row.allocationId = mapRequiredId(maps, "global_port_allocations", source.allocationId);
      row.resourceId = mapGlobalPortResourceId(maps, source.resourceType, source.resourceId);
      row.hostId = mapOptionalId(maps, "hosts", source.hostId);
      row.ownerGroupTag = remapGlobalPortOwnerGroupTag(maps, source.ownerGroupTag);
      const sourceKey = String(source.referenceKey ?? "");
      const slot = sourceKey.endsWith(":PRIMARY") ? "PRIMARY" : sourceKey.endsWith(":MAPPED") ? "MAPPED" : undefined;
      row.referenceKey = buildGlobalPortReferenceKey({
        resourceType: String(source.resourceType) as GlobalPortResourceType,
        resourceId: row.resourceId,
        hostId: row.hostId,
        network: String(source.network) as GlobalPortReferenceNetwork,
        role: String(source.role) as GlobalPortReferenceRole,
        ...(slot ? { slot } : {}),
      });
      return { row, existingWhere: row.referenceKey ? { referenceKey: row.referenceKey } : undefined };

    case "global_port_probe_runs":
      row.allocationId = mapOptionalId(maps, "global_port_allocations", source.allocationId);
      row.createdByUserId = mapOptionalId(maps, "users", source.createdByUserId);
      return { row, existingWhere: row.probeTag ? { probeTag: row.probeTag } : undefined };

    case "global_port_probe_results":
      row.probeRunId = mapRequiredId(maps, "global_port_probe_runs", source.probeRunId);
      row.hostId = mapRequiredId(maps, "hosts", source.hostId);
      return { row, existingWhere: { probeRunId: row.probeRunId, hostId: row.hostId, network: row.network } };

    case "global_port_scan_leases":
      row.leaseOwnerHash = null;
      row.leaseUntil = null;
      return { row, existingWhere: { scopeKey: row.scopeKey } };
...
async function updateMappedColumn(
  table: string,
  column: string,
  updates: Array<{ id: number; value: number | string }>,
) {
...
async function fixDeferredQuickConfigReferences(
  snapshot: MigrationSnapshot,
  maps: ImportMaps,
  newlyInsertedQuickConfigIds: Set<number>,
  newlyInsertedQuickOperationIds: Set<number>,
  newlyInsertedGlobalPortAllocationIds: Set<number>,
) {
  const activeTopologyUpdates: Array<{ id: number; value: number }> = [];
  const desiredTopologyUpdates: Array<{ id: number; value: number }> = [];
  const currentOperationUpdates: Array<{ id: number; value: number }> = [];
  const retryOperationUpdates: Array<{ id: number; value: number }> = [];
  const allocationOwnerTagUpdates: Array<{ id: number; value: string }> = [];
  try {
    for (const source of sortedSnapshotRows(snapshot, "xray_quick_configs")) {
      const sourceId = toNumberId(source.id);
      if (!sourceId || !newlyInsertedQuickConfigIds.has(sourceId)) continue;
      const id = mapRequiredId(maps, "xray_quick_configs", sourceId);
      const activeTopologyRevisionId = mapOptionalId(
        maps, "xray_quick_config_topology_revisions", source.activeTopologyRevisionId,
      );
      const desiredTopologyRevisionId = mapOptionalId(
        maps, "xray_quick_config_topology_revisions", source.desiredTopologyRevisionId,
      );
      const currentOperationId = mapOptionalId(
        maps, "xray_quick_config_operations", source.currentOperationId,
      );
      if (activeTopologyRevisionId) activeTopologyUpdates.push({ id, value: activeTopologyRevisionId });
      if (desiredTopologyRevisionId) desiredTopologyUpdates.push({ id, value: desiredTopologyRevisionId });
      if (currentOperationId) currentOperationUpdates.push({ id, value: currentOperationId });
    }
    for (const source of sortedSnapshotRows(snapshot, "xray_quick_config_operations")) {
      const sourceId = toNumberId(source.id);
      if (!sourceId || !newlyInsertedQuickOperationIds.has(sourceId)) continue;
      const retryOfOperationId = mapOptionalId(
        maps, "xray_quick_config_operations", source.retryOfOperationId,
      );
      if (retryOfOperationId) {
        retryOperationUpdates.push({
          id: mapRequiredId(maps, "xray_quick_config_operations", sourceId),
          value: retryOfOperationId,
        });
      }
    }
    for (const source of sortedSnapshotRows(snapshot, "global_port_allocations")) {
      const sourceId = toNumberId(source.id);
      if (!sourceId || !newlyInsertedGlobalPortAllocationIds.has(sourceId)
        || source.primaryOwnerTag === null || source.primaryOwnerTag === undefined) continue;
      allocationOwnerTagUpdates.push({
        id: mapRequiredId(maps, "global_port_allocations", sourceId),
        value: remapGlobalPortOwnerGroupTag(maps, source.primaryOwnerTag),
      });
    }
    await withDatabaseTransaction(async () => {
      await updateMappedColumn("xray_quick_configs", "activeTopologyRevisionId", activeTopologyUpdates);
      await updateMappedColumn("xray_quick_configs", "desiredTopologyRevisionId", desiredTopologyUpdates);
      await updateMappedColumn("xray_quick_configs", "currentOperationId", currentOperationUpdates);
      await updateMappedColumn("xray_quick_config_operations", "retryOfOperationId", retryOperationUpdates);
      await updateMappedColumn("global_port_allocations", "primaryOwnerTag", allocationOwnerTagUpdates);
    });
  } catch {
    throw new XraySecretUnavailableError();
  }
...
async function resetImportedRuntimeState(
  maps: ImportMaps,
  options: { resetCopiedQuickConfigRuntime?: boolean } = {},
) {
  ...
  const updateByHostIds = async (table: string, patchSql: string, params: unknown[] = []) => {
    if (hostIds.length === 0) return;
    for (const chunk of chunkValues(hostIds, MIGRATION_ID_BATCH_SIZE)) {
      await executeRaw(
        `UPDATE ${quote(table)} SET ${patchSql} WHERE ${quote("hostId")} IN (${chunk.map(() => "?").join(", ")})`,
        [...params.map(normalizeValue), ...chunk],
      );
    }
  };
  await withDatabaseTransaction(async () => {
    ...
    await updateByHostIds("xray_managed_service_runtime_reports", [
      `${quote("stateJson")} = NULL`,
      `${quote("stateSignature")} = NULL`,
      `${quote("reportedAt")} = NULL`,
      `${quote("updatedAt")} = ?`,
    ].join(", "), [now]);
    await updateByHostIds("xray_managed_service_deployments", [
      `${quote("desiredGeneration")} = ${quote("desiredGeneration")} + 1`,
      `${quote("desiredConfigHash")} = NULL`,
      `${quote("updatedAt")} = ?`,
    ].join(", "), [now]);
    if (options.resetCopiedQuickConfigRuntime) {
      await executeRaw(
        `UPDATE ${quote("xray_quick_config_operations")} SET ${quote("executionOwnerId")} = NULL, ${quote("executionLeaseUntil")} = NULL WHERE ${quote("executionOwnerId")} IS NOT NULL OR ${quote("executionLeaseUntil")} IS NOT NULL`,
      );
      await executeRaw(
        `UPDATE ${quote("xray_quick_config_operation_steps")} SET ${quote("status")} = 'PENDING', ${quote("resultSummaryJson")} = NULL, ${quote("errorCode")} = NULL, ${quote("startedAt")} = NULL, ${quote("finishedAt")} = NULL WHERE ${quote("status")} = 'RUNNING'`,
      );
      await executeRaw(
        `UPDATE ${quote("global_port_scan_leases")} SET ${quote("leaseOwnerHash")} = NULL, ${quote("leaseUntil")} = NULL, ${quote("updatedAt")} = ? WHERE ${quote("scopeKey")} = ?`,
        [now, "GLOBAL_PORT_RECLAIM"],
      );
    }
  });
}
...
export async function importDirectSqliteBackup(
  ...
  await withSqliteExclusive((sqlite) => {
    ...
    try {
      ...
      if (sourceTables.has("xray_inbounds")) {
        runtimeSnapshot.tables.xray_inbounds = sqlite.prepare("SELECT * FROM migration_source.xray_inbounds").all() as Record<string, any>[];
      }
      if (sourceTables.has("xray_clients")) {
        runtimeSnapshot.tables.xray_clients = sqlite.prepare("SELECT * FROM migration_source.xray_clients").all() as Record<string, any>[];
      }
      for (const table of [
        "xray_tls_certificates",
        "xray_operations",
        ...
        if (sourceTables.has(table)) {
          runtimeSnapshot.tables[table] = sqlite.prepare(`SELECT * FROM migration_source.${sqliteIdentifier(table)}`).all() as Record<string, any>[];
        }
      }
      for (const table of XRAY_GENERIC_SECRET_TABLE_NAMES) {
        if (sourceTables.has(table)) {
          runtimeSnapshot.tables[table] = sqlite.prepare(`SELECT * FROM migration_source.${sqliteIdentifier(table)}`).all() as Record<string, any>[];
        }
      }
      for (const table of DNS_PROVIDER_TABLE_NAMES) {
        if (sourceTables.has(table)) {
          runtimeSnapshot.tables[table] = sqlite.prepare(`SELECT * FROM migration_source.${sqliteIdentifier(table)}`).all() as Record<string, any>[];
        }
      }
      for (const table of QUICK_CONFIG_TABLE_NAMES) {
        if (sourceTables.has(table)) {
          runtimeSnapshot.tables[table] = sqlite.prepare(`SELECT * FROM migration_source.${sqliteIdentifier(table)}`).all() as Record<string, any>[];
        }
      }
      assertMigrationSnapshotXraySecretsAvailable(runtimeSnapshot);
      ...
      try {
        ...
        for (const table of tableDefs) {
          ...
          const sourceColumnRows = sqlite.prepare(
            `PRAGMA migration_source.table_info(${sqliteIdentifier(table.name)})`,
          ).all() as Array<{ name: string }>;
          const sourceColumns = new Set(sourceColumnRows.map((column) => String(column.name)));
          const selectedColumns = table.columns.map((column) => {
            if (sourceColumns.has(column.name)) return sqliteIdentifier(column.name);
            if (table.name === "xray_managed_service_accounts" && column.name === "settingsVersion") {
              return `1 AS ${sqliteIdentifier(column.name)}`;
            }
            if (table.name === "xray_managed_service_accounts" && column.name === "settingsJson") {
              return `'{}' AS ${sqliteIdentifier(column.name)}`;
            }
            if ((table.name === "xray_inbounds" && column.name === "externalProxyNodeId")
              || (table.name === "forward_rules" && ["targetExternalProxyNodeId", "xrayQuickConfigId"].includes(column.name))) {
              return `NULL AS ${sqliteIdentifier(column.name)}`;
            }
            throw new Error(`源 SQLite 数据表 ${table.name} 缺少字段 ${column.name}`);
          }).join(", ");
          sqlite.exec(
            `INSERT INTO main.${sqliteIdentifier(table.name)} (${columns}) SELECT ${selectedColumns} FROM migration_source.${sqliteIdentifier(table.name)}`,
          );
        }
  ...
  await ensureDatabaseSchema();
  await backfillGlobalPortAllocations();
  ...
  await resetImportedRuntimeState(maps, { resetCopiedQuickConfigRuntime: true });
...
export async function importMigrationSnapshot(
  ...
  assertMigrationSnapshotXraySecretsAvailable(snapshot);
  ...
  const maps: ImportMaps = {};
  const newlyInsertedQuickConfigIds = new Set<number>();
  const newlyInsertedQuickOperationIds = new Set<number>();
  const newlyInsertedGlobalPortAllocationIds = new Set<number>();
  ...
  const rememberNewQuickRow = (table: string, oldId: number) => {
    if (table === "xray_quick_configs") newlyInsertedQuickConfigIds.add(oldId);
    if (table === "xray_quick_config_operations") newlyInsertedQuickOperationIds.add(oldId);
    if (table === "global_port_allocations") newlyInsertedGlobalPortAllocationIds.add(oldId);
  };
  ...
  const hasDnsProviderRows = DNS_PROVIDER_TABLE_NAMES.some(
    (table) => (snapshot.tables?.[table]?.length || 0) > 0,
  );
  let dnsProviderTablesImported = false;
  const importDnsProviderTables = async () => {
    try {
      await withDatabaseTransaction(async () => {
        const dnsBindingPlan = await planDnsProviderBindingImport(snapshot, mode);
        for (const table of DNS_PROVIDER_TABLE_NAMES) {
          const rows = sortedSnapshotRows(snapshot, table);
          if (rows.length === 0) continue;
          reportRowProgress(table, true);

          if (table === "dns_provider_global_bindings") {
            const source = rows[0];
            if (!dnsBindingPlan || toNumberId(source.id) !== dnsBindingPlan.sourceId) {
              throw new XraySecretUnavailableError();
            }
            const accountId = mapOptionalId(maps, "dns_provider_accounts", source.accountId);
            if (dnsBindingPlan.action === "restore-seed") {
              const sourceRevision = Number(source.revision);
              if (!Number.isSafeInteger(sourceRevision) || sourceRevision <= 0) {
                throw new XraySecretUnavailableError();
              }
              const result = await executeRaw(
                `UPDATE ${quote(table)} SET ${quote("accountId")} = ?, ${quote("revision")} = ?, ${quote("createdAt")} = ?, ${quote("updatedAt")} = ? WHERE ${quote("id")} = ? AND ${quote("scopeKey")} = ? AND ${quote("accountId")} IS NULL AND ${quote("revision")} = ?`,
                [
                  accountId,
                  sourceRevision,
                  source.createdAt,
                  source.updatedAt,
                  dnsBindingPlan.targetId,
                  "XRAY_QUICK_CONFIG",
                  dnsBindingPlan.targetRevision,
                ],
              );
              if (changedRowCount(result) !== 1) throw new XraySecretUnavailableError();
              incrementCounter(updated, table);
            } else if (dnsBindingPlan.action === "bind-empty") {
              if (!accountId) throw new XraySecretUnavailableError();
              const result = await executeRaw(
                `UPDATE ${quote(table)} SET ${quote("accountId")} = ?, ${quote("revision")} = ${quote("revision")} + 1, ${quote("updatedAt")} = ? WHERE ${quote("id")} = ? AND ${quote("scopeKey")} = ? AND ${quote("accountId")} IS NULL AND ${quote("revision")} = ?`,
                [
                  accountId,
                  nowDate(),
                  dnsBindingPlan.targetId,
            ...
            maps[table].set(dnsBindingPlan.sourceId, dnsBindingPlan.targetId);
            processed += 1;
            reportRowProgress(table, true);
            continue;
          }
          ...
          if (mode === "restore") {
            ...
            for (const source of rows) {
              ...
              if (!oldId) throw new XraySecretUnavailableError();
              const prepared = await prepareImportRow(table, source, maps);
              if (!prepared) throw new XraySecretUnavailableError();
              preparedRows.push({
                source,
                oldId,
            ...
            await insertRestoreRowsStrict(table, preparedRows, (row) => {
          ...
          const destinationRows = await queryRaw(`SELECT 1 AS present FROM ${quote(table)} LIMIT 1`);
          const destinationHasRows = destinationRows.length > 0;
          ...
          const existingIds = destinationHasRows
            ? await preloadExistingImportIds(table, preparedGeneratedRows)
            : new Map<string, number>();
          const generatedRows: PreparedGeneratedRow[] = [];
          for (const row of preparedGeneratedRows) {
            const descriptor = destinationHasRows ? importLookupDescriptor(row.existingWhere) : null;
            const existingId = descriptor ? existingIds.get(descriptor.key) || null : null;
            if (existingId) {
          ...
          await insertGeneratedRowsStrict(table, generatedRows, mysqlStep, (row, newId) => {
      ...
      if (error instanceof XraySecretUnavailableError) throw error;
      throw new XraySecretUnavailableError();
    }
  };
  ...
  for (const table of IMPORT_TABLE_ORDER) {
    if ((DNS_PROVIDER_TABLE_NAMES as readonly string[]).includes(table)) {
      if (!dnsProviderTablesImported) {
        dnsProviderTablesImported = true;
        if (hasDnsProviderRows) await importDnsProviderTables();
      }
      continue;
    }
    const rows = sortedSnapshotRows(snapshot, table);
    ...
    if (table === "global_port_scan_leases") {
      const source = rows[0];
      const sourceId = toNumberId(source?.id);
      const targetRows = await queryRaw<Record<string, any>>(
        `SELECT ${quote("id")}, ${quote("scopeKey")}, ${quote("leaseOwnerHash")}, ${quote("leaseUntil")}, ${quote("lastStartedAt")}, ${quote("lastFinishedAt")} FROM ${quote(table)}`,
      );
      const targetId = toNumberId(targetRows[0]?.id);
      if (rows.length !== 1 || !sourceId || source.scopeKey !== "GLOBAL_PORT_RECLAIM"
        || targetRows.length !== 1 || !targetId || targetRows[0].scopeKey !== "GLOBAL_PORT_RECLAIM") {
        throw new XraySecretUnavailableError();
      }
      await executeRaw(
        `UPDATE ${quote(table)} SET ${quote("leaseOwnerHash")} = ?, ${quote("leaseUntil")} = ?, ${quote("lastStartedAt")} = ?, ${quote("lastFinishedAt")} = ?, ${quote("updatedAt")} = ? WHERE ${quote("id")} = ? AND ${quote("scopeKey")} = ?`,
        [
          mode === "restore" ? null : targetRows[0].leaseOwnerHash ?? null,
          mode === "restore" ? null : targetRows[0].leaseUntil ?? null,
          mode === "restore" ? source.lastStartedAt ?? null : targetRows[0].lastStartedAt ?? null,
          mode === "restore" ? source.lastFinishedAt ?? null : targetRows[0].lastFinishedAt ?? null,
          nowDate(), targetId, "GLOBAL_PORT_RECLAIM",
        ],
      );
      maps[table].set(sourceId, targetId);
      incrementCounter(reused, table);
      processed += 1;
      reportRowProgress(table, true);
      continue;
    ...
    const strictQuickConfigTable = (QUICK_CONFIG_TABLE_NAMES as readonly string[]).includes(table)
      || (table === "forward_rules" && rows.some((row) => row.xrayQuickConfigId !== null && row.xrayQuickConfigId !== undefined));

    if (mode === "restore" && table !== "users" && table !== "system_settings") {
      if (strictQuickConfigTable) {
        const preparedRows: PreparedRestoreRow[] = [];
        for (const source of rows) {
          const oldId = toNumberId(source.id);
          if (!oldId) throw new XraySecretUnavailableError();
        ...
        try {
          await insertRestoreRowsStrict(table, preparedRows, (row) => {
            maps[table].set(row.oldId, row.oldId);
            rememberNewQuickRow(table, row.oldId);
            incrementCounter(inserted, table);
            processed += 1;
            reportRowProgress(table);
          });
  ...
  await fixDeferredQuickConfigReferences(
    snapshot,
    maps,
    newlyInsertedQuickConfigIds,
    newlyInsertedQuickOperationIds,
    newlyInsertedGlobalPortAllocationIds,
  );
  // Global port ownership for forward-group children is derived from the
  // restored parent rule. Resolve those deferred pointers before seeding the
  // ledger, and fail closed if any relationship cannot be restored.
  await fixDeferredForwardGroupReferences(snapshot, maps);
  await backfillGlobalPortAllocations();
  await repairXrayClientFingerprints({ force: true });
