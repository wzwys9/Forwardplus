...
async function githubAssetApiUrl(version: string, asset: string) {
  const releaseUrl = `https://api.github.com/repos/wzwys9/Forwardplus/releases/tags/${encodeURIComponent(`v${normalizeVersion(version)}`)}`;
  const release = await fetch(releaseUrl, withGithubAuth(releaseUrl, {
    cache: "no-store",
    headers: {
  ...
  const payload = await release.json() as { assets?: Array<{ name?: string; url?: string; state?: string; size?: number }> };
  const candidate = (payload.assets || []).find((item) => item.name === asset);
  if (!candidate?.url || candidate.state !== "uploaded" || Number(candidate.size || 0) <= 0) return null;
...
async function downloadAgentAssetToCache(version: string, asset: string) {
  ...
  for (const releaseVersion of getAgentAssetReleaseCandidates(normalized)) {
    ...
    try {
      const assetUrl = await githubAssetApiUrl(releaseVersion, asset);
      if (!assetUrl) continue;
      const res = await fetch(assetUrl, withGithubAuth(assetUrl, {
        cache: "no-store",
        redirect: "follow",
        headers: {
