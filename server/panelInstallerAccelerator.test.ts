...
`,
    ...
    assert.deepEqual(result.calls, [
      "https://mirror.example.com/https://api.github.com/repos/wzwys9/Forwardplus/releases/latest",
      "https://api.github.com/repos/wzwys9/Forwardplus/releases/latest",
    ]);
...
code="$(download_github_archive 'https://github.com/wzwys9/Forwardplus/releases/download/v9.8.7/forwardx-panel-v9.8.7.tar.gz' "$OUTPUT_FILE")"
printf 'CODE=%s\\n' "$code"
`,
  });
  assert.equal(result.status, 0, result.stderr);
  ...
  assert.deepEqual(result.calls, [
    "https://mirror.example.com/https://github.com/wzwys9/Forwardplus/releases/download/v9.8.7/forwardx-panel-v9.8.7.tar.gz",
    "https://github.com/wzwys9/Forwardplus/releases/download/v9.8.7/forwardx-panel-v9.8.7.tar.gz",
  ]);
...
test("both installers persist the accelerator and Docker keeps GHCR image pulls unchanged", () => {
  for (const installer of installers) {
    ...
    assert.match(installer.source, /FORWARDPLUS_GITHUB_TOKEN=\$GITHUB_TOKEN/);
    assert.match(installer.source, /chmod 600 "\$APP_DIR\/\.env"/);
    assert.match(installer.source, /--config "\$auth_config"/);
    assert.doesNotMatch(installer.source, /-H "Authorization: Bearer \$\{GITHUB_TOKEN\}"/);
  }
  assert.match(dockerSource, /FORWARDPLUS_GITHUB_TOKEN: \$\{FORWARDPLUS_GITHUB_TOKEN:-\}/);
...
test("installer tokens are sent only to HTTPS GitHub endpoints", { skip: !bash }, () => {
  for (const installer of installers) {
    const result = runHarness({
      installer,
      body: `
for url in https://api.github.com/repos/x/y http://api.github.com/repos/x/y https://example.com/file; do
  if is_official_github_url "$url"; then printf 'yes\n'; else printf 'no\n'; fi
done
`,
    });
    assert.equal(result.status, 0, result.stderr);
