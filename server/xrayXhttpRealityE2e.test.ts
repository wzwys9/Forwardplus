...
test("VLESS XHTTP Reality carries a real HTTPS request through fixed Xray", { timeout: 30_000 }, async (t) => {
  const binary = process.env.XRAY_TEST_BINARY;
  if (!binary) return t.skip("XRAY_TEST_BINARY is not configured");
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "forwardx-xray-xhttp-e2e-"));
  ...
  const inbound: XrayConfigInboundInput = {
    ...
    clients: [{
      ...
      uuid,
      shortId,
      statsKey: "forwardx-xhttp-e2e-client",
      flow: "",
      isEnabled: true,
      pendingDelete: false,
      sortOrder: 0,
