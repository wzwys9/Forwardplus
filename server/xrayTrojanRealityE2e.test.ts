...
test("Trojan RAW Reality carries a real HTTPS request through fixed Xray", { timeout: 30_000 }, async (t) => {
  const binary = process.env.XRAY_TEST_BINARY;
  if (!binary) return t.skip("XRAY_TEST_BINARY is not configured");
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "forwardx-xray-trojan-e2e-"));
  ...
  const inbound: XrayConfigInboundInput = {
    id: 1,
    runtimeTag: "forwardx-trojan-e2e",
    ...
    clients: [{
      ...
      password,
      shortId,
      statsKey: "forwardx-trojan-e2e-client",
      isEnabled: true,
      pendingDelete: false,
      sortOrder: 0,
    }],
  ...
  fs.writeFileSync(clientConfigPath, `${JSON.stringify({
    ...
    outbounds: [{
      tag: "proxy",
      protocol: "trojan",
      settings: { servers: [{ address: "127.0.0.1", port: serverPort, password }] },
      streamSettings: {
        network: "tcp",
        security: "reality",
        realitySettings: {
